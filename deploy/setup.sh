#!/usr/bin/env bash
# VM bootstrap script for ThreatClaw on GCP Compute Engine.
#
# Run on a fresh Debian 12 VM after SSH:
#   sudo bash setup.sh
#
# Prerequisites:
#   - VM has the threatclaw-vm service account attached
...
cp /tmp/deploy/threatclaw.service /etc/systemd/system/
...
# before dropping to uid 1000 (threatclaw) inside the container.
mkdir -p /opt/threatclaw
chmod 700 /opt/threatclaw

if [ ! -f /opt/threatclaw/.env ]; then
  echo "WARNING: /opt/threatclaw/.env does not exist."
  echo "Create it with your configuration before starting ThreatClaw."
  echo "See deploy/env.example for the required variables."
  echo ""
  echo "Then run: systemctl enable threatclaw && systemctl start threatclaw"
else
  chmod 600 /opt/threatclaw/.env
  echo "==> Starting ThreatClaw"
  systemctl enable threatclaw
  systemctl start threatclaw
fi
...
echo "  systemctl status threatclaw"
echo "  docker logs threatclaw"
