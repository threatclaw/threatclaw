...
**Goal:** Build a Python + Playwright E2E testing framework that exercises the ThreatClaw web gateway through a real browser against the real binary with a mock LLM backend.

**Architecture:** pytest session fixtures start a mock OpenAI-compat HTTP server and the threatclaw binary (libSQL in-memory, gateway enabled), then per-test Playwright browser instances navigate to the gateway and make DOM assertions.
...
- Support `stream: true` with proper SSE chunk format (critical for ThreatClaw's streaming)
...
- ThreatClaw logs `Web UI: http://{host}:{port}/` to stdout (main.rs:508) using the config port, not the bound port. So we must use a fixed port, not port 0.
...
Session-scoped: build binary, start mock LLM, start threatclaw.
...
@pytest.fixture(scope="session")
async def threatclaw_server(threatclaw_binary, mock_llm_server):
...
@pytest.fixture
async def page(threatclaw_server):
...
git commit -m "feat: E2E conftest with session fixtures for mock LLM and threatclaw"
```
...
Browser-level end-to-end tests for the ThreatClaw web gateway using Python + Playwright.

## Prerequisites

- Python 3.11+
- Rust toolchain (for building threatclaw)
...
The tests need the threatclaw binary built with libsql support:
...
2. **ThreatClaw** -- the real binary with gateway enabled, pointing to the mock LLM
