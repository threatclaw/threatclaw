...
**Date:** 2026-02-24
**Status:** Approved
**Goal:** Deterministic browser-level E2E tests for the ThreatClaw web gateway using Python + Playwright, with a mock LLM backend for CI reliability.
...
2. Session-scoped fixture builds threatclaw binary (or reuses cached)
3. Session-scoped fixture starts mock LLM on OS-assigned port
4. Session-scoped fixture starts threatclaw subprocess pointing to mock LLM, gateway on OS-assigned port, libSQL in-memory
...
7. Teardown kills threatclaw and mock LLM
...
```
tests/e2e/
  conftest.py              # pytest fixtures: build binary, start threatclaw, mock LLM, browser
...
- Supports `stream: true` by returning SSE chunks with `delta` objects (critical: ThreatClaw streams responses via SSE to the browser)
...
**`threatclaw_binary`**
- Checks if `./target/debug/threatclaw` exists
...
**`threatclaw_server(threatclaw_binary, mock_llm_server)`**
- Starts the threatclaw binary with environment:
...
- Parses actual gateway port from threatclaw stdout (`Gateway listening on 127.0.0.1:XXXX`)
...
ThreatClaw logs `Gateway listening on 127.0.0.1:XXXX` at startup. The fixture reads stdout line-by-line until it finds this pattern, extracts the port.
...
async def wait_for_port(process, pattern=r"Gateway listening on .+:(\d+)", timeout=60):
    ...
    raise TimeoutError("threatclaw did not report listening port")
```
...
1. `pytest tests/e2e/ -v` passes locally with a pre-built threatclaw binary
