# Automated QA Plan for ThreatClaw
...
A review of all closed issues and merged bug-fix PRs reveals that most ThreatClaw bugs fall into a few recurring categories:
...
Add the same validation for WASM tools (loaded from `~/.threatclaw/tools/`) and MCP tools (mock a simple MCP manifest and validate the schema it produces).
...
docker-build:
  ...
  steps:
    ...
    - name: Build Docker image
      run: docker build -t threatclaw-test:ci .
```
...
**Cost:** High (requires Anthropic computer use API, headless browser, threatclaw running)
...
```
+------------------+     +-----------------+     +------------------+
|  Test Runner     |     |  Headless       |     |  ThreatClaw        |
...
1. **Test runner** -- Python or TypeScript script that orchestrates the flow. Starts threatclaw, waits for readiness, launches Playwright browser, runs scenarios.
...
tests/
  e2e/
    conftest.py             # pytest fixtures: start threatclaw, browser
    ...
    Dockerfile.test         # Container for CI: threatclaw + chromium
```

**Fixture: start threatclaw**

```python
@pytest.fixture(scope="session")
async def threatclaw_server():
    """Start threatclaw with gateway enabled, return base URL."""
    env = {
        "CLI_ENABLED": "false",
...
```python
@pytest.fixture
async def browser_agent(threatclaw_server):
    """Playwright browser + Claude computer use agent."""
    async with async_playwright() as p:
        browser = await p.chromium.launch(headless=True)
        page = await browser.new_page(viewport={"width": 1280, "height": 720})
        await page.goto(f"{threatclaw_server}/?token=test-token-e2e")
...
```python
async def test_sse_reconnect_preserves_history(browser_agent, threatclaw_server):
    ...
    await restart_threatclaw(threatclaw_server)
...
```python
async def test_onboarding_wizard_completes(tmp_threatclaw_home):
    """Bugs: #187, #174, #129, #185 (wizard persistence and re-trigger)"""
    # Start threatclaw with a fresh home directory (no prior config)
    ...
    proc = pexpect.spawn(
        "cargo run",
        env={"THREATCLAW_HOME": str(tmp_threatclaw_home), **base_env},
        timeout=60,
    )

    # Step through wizard
    proc.expect("Welcome to ThreatClaw")
    ...
    proc2.expect("Agent threatclaw ready")  # Should skip wizard
    # Must NOT see "Welcome to ThreatClaw" again
    assert not proc2.match_any(["Welcome to ThreatClaw"], timeout=5)
    proc2.close()
```
