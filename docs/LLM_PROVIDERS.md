...
ThreatClaw defaults to NEAR AI for model access, but supports any OpenAI-compatible
...
No additional configuration required. On first run, `threatclaw onboard` opens a browser
for OAuth authentication. Credentials are saved to `~/.threatclaw/session.json`.
