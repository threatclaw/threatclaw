...
This guide covers configuring the Telegram channel for ThreatClaw, including DM pairing for access control.

## Overview

The Telegram channel lets you interact with ThreatClaw via Telegram DMs and groups. It supports:
...
- ThreatClaw installed and configured (`threatclaw onboard`)
...
2. Bot replies: `To pair with this bot, run: threatclaw pairing approve telegram ABC12345`
3. You run: `threatclaw pairing approve telegram ABC12345`
...
# List as JSON
threatclaw pairing list telegram --json
...
Edit `~/.threatclaw/channels/telegram.capabilities.json` (or the config injected by the host):
...
# Install
mkdir -p ~/.threatclaw/channels
cp channels-src/telegram/telegram.wasm channels-src/telegram/telegram.capabilities.json ~/.threatclaw/channels/
```
...
- **Secrets store**: `threatclaw` CLI (if available)
...
- Set `bot_username` in config to your bot's username (e.g., `MyThreatClawBot`)
- Ensure the message contains `@YourBot` or starts with `/`

### "Connection refused" when starting

- For webhook mode: Start your tunnel before `threatclaw run`
