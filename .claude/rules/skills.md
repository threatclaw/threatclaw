...
|-------------|--------|-------------|
| **Trusted** | User-placed in `~/.threatclaw/skills/` or workspace `skills/` | All tools available to the agent |
| **Installed** | Downloaded from ClawHub registry (`~/.threatclaw/installed_skills/`) | Read-only tools only (no shell, file write, HTTP) |
