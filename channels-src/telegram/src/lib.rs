...
//! Telegram Bot API channel for ThreatClaw.
//!
...
fn send_photo(
    ...
    let boundary = format!("threatclaw-{}", channel_host::now_millis());
...
fn send_pairing_reply(chat_id: i64, code: &str) -> Result<(), String> {
    send_message(
        chat_id,
        &format!(
            "To pair with this bot, run: `threatclaw pairing approve telegram {}`",
            code
        ),
        None,
        Some("Markdown"),
