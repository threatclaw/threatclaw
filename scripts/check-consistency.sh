...
EXPECTED_INSTALLER_SHA="aca02a77c448d90d00bf7581b359517264301f24fe3c455bd476767b18b2ac16"
if [ -f installer/install-agent.ps1 ]; then
  ...
  if [ "$ACTUAL_INSTALLER_SHA" = "$EXPECTED_INSTALLER_SHA" ]; then
    ...
    fail "install-agent.ps1 changed ($ACTUAL_INSTALLER_SHA) — re-capture the reassembled 4104 hash (INSTALLER_OSQUERY_PS_SHA256 in src/agent/detection_provenance.rs, procedure in its doc comment) and update EXPECTED_INSTALLER_SHA in this script"
  fi
fi
