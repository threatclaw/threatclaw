<p align="center">
  <img src="threatclaw.png?v=2" alt="ThreatClaw" width="200"/>
</p>
...
<p align="center">
  ...
  <a href="https://t.me/threatclawAI"><img src="https://img.shields.io/badge/Telegram-%40threatclawAI-26A5E4?style=flat&logo=telegram&logoColor=white" alt="Telegram: @threatclawAI" /></a>
  <a href="https://www.reddit.com/r/threatclawAI/"><img src="https://img.shields.io/badge/Reddit-r%2FthreatclawAI-FF4500?style=flat&logo=reddit&logoColor=white" alt="Reddit: r/threatclawAI" /></a>
</p>
...
ThreatClaw 基于一个简单的原则：**你的 AI 助手应该为你服务，而不是与你为敌。**

在 AI 系统对数据处理日益不透明、与企业利益捆绑的今天，ThreatClaw 选择了一条不同的路：
...
ThreatClaw 是一个你真正可以信赖的 AI 助手，无论是个人生活还是工作。
...
- **动态工具构建** — 描述你的需求，ThreatClaw 会将其构建为 WASM 工具
...
访问 [Releases 页面](https://github.com/nearai/threatclaw/releases/) 查看最新版本。

<details>
  <summary>通过 Windows 安装程序安装 (Windows)</summary>

下载 [Windows 安装程序](https://github.com/nearai/threatclaw/releases/latest/download/threatclaw-x86_64-pc-windows-msvc.msi) 并运行。
...
```sh
irm https://github.com/nearai/threatclaw/releases/latest/download/threatclaw-installer.ps1 | iex
```
...
```sh
curl --proto '=https' --tlsv1.2 -LsSf https://github.com/nearai/threatclaw/releases/latest/download/threatclaw-installer.sh | sh
```
</details>
...
# 启用 pgvector 扩展
psql threatclaw -c "CREATE EXTENSION IF NOT EXISTS vector;"
```

## 配置

运行设置向导来配置 ThreatClaw：

```bash
threatclaw onboard
```

向导将引导你完成数据库连接、NEAR AI 身份验证（通过浏览器 OAuth）和密钥加密（使用系统钥匙串）。设置会保存在数据库中；引导变量（如 `DATABASE_URL`、`LLM_BACKEND`）写入 `~/.threatclaw/.env`，以便在数据库连接前可用。

### 替代 LLM 提供商

ThreatClaw 默认使用 NEAR AI，但开箱即用地支持多种 LLM 提供商。
...
ThreatClaw 实现了纵深防御策略来保护你的数据并防止滥用。
...
# 启用调试日志
RUST_LOG=threatclaw=debug cargo run
```
...
ThreatClaw 是受 [OpenClaw](https://github.com/openclaw/openclaw) 启发的 Rust 重新实现。参见 [FEATURE_PARITY.md](FEATURE_PARITY.md) 了解完整的功能追踪矩阵。
