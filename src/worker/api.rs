...
//!
//! Every request includes a bearer token from `THREATCLAW_WORKER_TOKEN` env var.
...
impl WorkerHttpClient {
    ...
    ///
    /// Reads `THREATCLAW_WORKER_TOKEN` from the environment.
    pub fn from_env(orchestrator_url: String, job_id: Uuid) -> Result<Self, WorkerError> {
        let token =
            std::env::var("THREATCLAW_WORKER_TOKEN").map_err(|_| WorkerError::MissingToken)?;
