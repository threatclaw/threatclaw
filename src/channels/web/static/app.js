...
function authenticate() {
  ...
  apiFetch('/api/chat/threads')
    .then(() => {
      sessionStorage.setItem('threatclaw_token', token);
    ...
    .catch(() => {
      sessionStorage.removeItem('threatclaw_token');
...
(function autoAuth() {
  ...
  const saved = sessionStorage.getItem('threatclaw_token');
  if (saved) {
...
function fetchGatewayStatus() {
  apiFetch('/api/gateway/status').then(function(data) {
    ...
    // Version
    if (data.version) {
      html += '<div class="gw-section-label">ThreatClaw v' + escapeHtml(data.version) + '</div>';
