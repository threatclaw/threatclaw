...
pub async fn start_server(
    ...
    let protected = Router::new()
        ...
        .route(
            ...
            get(super::handlers::licensing_api::premium_status_handler),
        )
        .route(
        ...
        .route(
            ...
            post(super::handlers::threatclaw_api::incidents_bulk_archive_stale_handler),
        )
        .route(
...
///
/// Used on hosted instances where `THREATCLAW_OAUTH_CALLBACK_URL` points to
...
async fn oauth_callback_handler(
    ...
    let state_param = match params.get("state") {
        ...
        _ => {
            clear_auth_mode(&state).await;
            return oauth_error_page("ThreatClaw");
        }
    };
    ...
    let exchange_proxy_url = std::env::var("THREATCLAW_OAUTH_EXCHANGE_URL").ok();
    ...
    if success {
        axum::response::Html(
            "<html><body style='font-family: system-ui; text-align: center; padding: 60px;'>\
             ...
             <p>You can close this tab and return to ThreatClaw.</p>\
                ...
                crate::extensions::ExtensionSource::WasmBuildable { .. } => {
                    format!(
                        "'{}' requires building from source. \
                         Run `threatclaw registry install {}` from the CLI.",
                        req.name, req.name
                    )
                }
                _ => format!(
...
/// Shared logic: resolve the file inside `~/.threatclaw/projects/{project_id}/`,
...
async fn serve_project_file(project_id: &str, path: &str) -> axum::response::Response {
    ...
    let base = threatclaw_base_dir().join("projects").join(project_id);
...
async fn gateway_status_handler(
    ...
    let restart_enabled = std::env::var("THREATCLAW_IN_DOCKER")
