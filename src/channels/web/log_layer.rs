...
/// (e.g. `threatclaw=debug`) without restarting the process.
...
impl LogLevelHandle {
    ...
    /// Change the `threatclaw=<level>` directive at runtime.
    ///
    ...
    pub fn set_level(&self, level: &str) -> Result<(), String> {
        ...
        let filter_str = if self.base_filter.is_empty() {
            format!("threatclaw={}", level)
        } else {
            format!("threatclaw={},{}", level, self.base_filter)
        };
    ...
    /// Returns the current threatclaw log level (e.g. "info", "debug").
...
pub fn init_tracing(log_broadcaster: Arc<LogBroadcaster>) -> Arc<LogLevelHandle> {
    let raw_filter =
        std::env::var("RUST_LOG").unwrap_or_else(|_| "threatclaw=info,tower_http=warn".to_string());

    // Split into the threatclaw directive and "everything else" (base_filter).
    let mut threatclaw_level = String::from("info");
    let mut base_parts: Vec<&str> = Vec::new();

    for part in raw_filter.split(',') {
        let trimmed = part.trim();
        if trimmed.starts_with("threatclaw=") {
            if let Some(lvl) = trimmed.strip_prefix("threatclaw=") {
                threatclaw_level = lvl.to_string();
            }
...
///   INFO threatclaw::agent: Request completed url="http://..." status=200
///
...
mod tests {
    ...
    fn test_log_broadcaster_subscribe() {
        ...
        broadcaster.send(LogEntry {
            ...
            target: "threatclaw::test".to_string(),
    ...
    fn test_log_entry_serialization() {
        let entry = LogEntry {
            ...
            target: "threatclaw::agent".to_string(),
