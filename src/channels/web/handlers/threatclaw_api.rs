...
// CORE-M1 — `incident_hitl_get_handler` (GET) supprimé : il exécutait la
// remédiation depuis un `?response=approve` avec un id entier devinable et sans
// nonce. Le dashboard utilise POST /api/tc/incidents/{id}/hitl ; les boutons de
// notification passent par le chemin nonce /api/tc/hitl/callback.
