...
#[cfg(unix)]
use threatclaw::channels::ChannelSecretUpdater;
#[cfg(any(feature = "postgres", feature = "libsql"))]
use threatclaw::setup::{SetupConfig, SetupWizard};
...
fn main() -> anyhow::Result<()> {
    ...
    threatclaw::bootstrap::load_threatclaw_env();
...
async fn async_main() -> anyhow::Result<()> {
    ...
    match &cli.command {
        ...
        Some(Command::Config(config_cmd)) => {
            init_cli_tracing();
            return threatclaw::cli::run_config_command(config_cmd.clone()).await;
        }
        Some(Command::Registry(registry_cmd)) => {
            init_cli_tracing();
            return threatclaw::cli::run_registry_command(registry_cmd.clone()).await;
        }
        Some(Command::Channels(channels_cmd)) => {
            init_cli_tracing();
            return threatclaw::cli::run_channels_command(
                channels_cmd.clone(),
                cli.config.as_deref(),
            )
            .await;
        ...
        Some(Command::Routines(routines_cmd)) => {
            init_cli_tracing();
            return threatclaw::cli::run_routines_cli(routines_cmd, cli.config.as_deref()).await;
        }
        ...
        Some(Command::Memory(mem_cmd)) => {
            init_cli_tracing();
            return threatclaw::cli::run_memory_command(mem_cmd).await;
        }
        ...
        Some(Command::Skills(skills_cmd)) => {
            init_cli_tracing();
            return threatclaw::cli::run_skills_command(skills_cmd.clone(), cli.config.as_deref())
                .await;
        }
        ...
        Some(Command::Doctor) => {
            init_cli_tracing();
            return threatclaw::cli::run_doctor_command().await;
        }
        Some(Command::Status) => {
            init_cli_tracing();
        ...
        Some(Command::Import(import_cmd)) => {
            init_cli_tracing();
            let config = threatclaw::config::Config::from_env().await?;
            return threatclaw::cli::run_import_command(import_cmd, &config).await;
        }
        Some(Command::Worker {
            job_id,
            orchestrator_url,
        ...
        Some(Command::ClaudeBridge {
            ...
            init_worker_tracing();
            return threatclaw::worker::run_claude_bridge(
                *job_id,
                orchestrator_url,
                *max_turns,
                model,
    ...
    let _pid_lock = match threatclaw::bootstrap::PidLock::acquire() {
        Ok(lock) => Some(lock),
        Err(threatclaw::bootstrap::PidLockError::AlreadyRunning { pid }) => {
            anyhow::bail!(
                "Another ThreatClaw instance is already running (PID {}). \
                 If this is incorrect, remove the stale PID file: {}",
                pid,
                threatclaw::bootstrap::pid_lock_path().display()
            );
        }
        Err(e) => {
    ...
    let config = match Config::from_env_with_toml(toml_path).await {
        Ok(c) => c,
        Err(threatclaw::error::ConfigError::MissingRequired { key, hint }) => {
            anyhow::bail!(
                "Configuration error: Missing required setting '{}'. {}. \
                 Run 'threatclaw onboard' to configure, or set the required environment variables.",
                key,
                hint
            );
        }
    ...
    let (config, active_tunnel) = threatclaw::tunnel::start_managed_tunnel(config).await;

    // ── Orchestrator / container job manager ────────────────────────────

    let orch = threatclaw::orchestrator::setup_orchestrator(
        &config,
        &components.llm,
    ...
    let shared_routine_engine_slot: threatclaw::channels::web::server::RoutineEngineSlot =
    ...
    if config.channels.wasm_channels_enabled && config.channels.wasm_channels_dir.exists() {
        let wasm_result = threatclaw::channels::wasm::setup_wasm_channels(
            &config,
    ...
    #[cfg(unix)]
    let mut http_channel_state: Option<Arc<threatclaw::channels::HttpChannelState>> = None;
    if !cli.cli_only
    ...
    let scheduler_slot: threatclaw::tools::builtin::SchedulerSlot =
        Arc::new(tokio::sync::RwLock::new(None));
    ...
    let mut sse_sender: Option<
        tokio::sync::broadcast::Sender<threatclaw::channels::web::types::SseEvent>,
    > = None;
    ...
    if config.channels.cli.enabled && cli.message.is_none() {
        let boot_info = threatclaw::boot_screen::BootInfo {
        ...
        threatclaw::boot_screen::print_boot_screen(&boot_info);
    }
    ...
    #[cfg(unix)]
    let sighup_settings_store: Option<Arc<dyn threatclaw::db::SettingsStore>> = components
        .db
        .as_ref()
    ...
    let deps = AgentDeps {
        ...
        transcription: config
            .transcription
            .create_provider()
            .map(|p| Arc::new(threatclaw::transcription::TranscriptionMiddleware::new(p))),
        document_extraction: Some(Arc::new(
            threatclaw::document_extraction::DocumentExtractionMiddleware::new(),
        )),
    };
            ...
            loop {
                ...
                if let Some(ref secrets_store) = sighup_secrets_store {
                        ...
                        .await
                    {
                        ...
                        threatclaw::config::inject_single_var(
                            "HTTP_WEBHOOK_SECRET",
                ...
                let new_config = match &sighup_settings_store_clone {
                    Some(store) => {
                        threatclaw::config::Config::from_db(store.as_ref(), &sighup_owner_id).await
                    }
                    None => threatclaw::config::Config::from_env().await,
                };
