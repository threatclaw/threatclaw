...
pub async fn sync_graylog(store: &dyn Database, config: &GraylogConfig) -> GraylogSyncResult {
    ...
    let mut req = client
        .post(format!("{}/api/events/search", url))
    ...
    if let Some(token) = &config.token {
        ...
        result
            .errors
            .push("No auth configured: set token or username+password".into());
        return result;
    }
    ...
    if !resp.status().is_success() {
        ...
        result
            .errors
            .push(format!("HTTP {}: {}", status, &text[..text.len().min(200)]));
        return result;
    }
    ...
    for event_wrapper in events {
        ...
        let source_ip = fields["source_ip"]
            .as_str()
            .or_else(|| fields["src_ip"].as_str());
        let hostname = fields["hostname"]
            .as_str()
        ...
        let rule_id = event["event_definition_id"]
            .as_str()
            .unwrap_or("graylog-event");
    ...
    tracing::info!(
        "GRAYLOG: Sync done — {} events, {} findings",
