...
pub async fn sync_dfir_iris(store: &dyn Database, config: &DfirIrisConfig) -> DfirIrisSyncResult {
    let mut result = DfirIrisSyncResult {
        cases_imported: 0, iocs_imported: 0, findings_created: 0, errors: vec![],
    };
    ...
    // Fetch cases
    let resp = match client.get(format!("{}/manage/cases/list", url))
        .header("Authorization", format!("Bearer {}", config.api_key))
        .header("Content-Type", "application/json")
        .send().await
    {
        Ok(r) => r,
        Err(e) => { result.errors.push(format!("Cases request: {}", e)); return result; }
    };

    if !resp.status().is_success() {
        let status = resp.status();
        let text = resp.text().await.unwrap_or_default();
        result.errors.push(format!("Cases HTTP {}: {}", status, &text[..text.len().min(200)]));
        return result;
    }

    let data: serde_json::Value = match resp.json().await {
        Ok(d) => d,
        Err(e) => { result.errors.push(format!("Parse cases: {}", e)); return result; }
    };
    ...
    for case in cases.iter().take(config.max_cases as usize) {
        ...
        match client.get(format!("{}/case/ioc/list", url))
            .header("Authorization", format!("Bearer {}", config.api_key))
            ...
            Ok(resp) if resp.status().is_success() => {
                if let Ok(ioc_data) = resp.json::<serde_json::Value>().await {
                    if let Some(iocs) = ioc_data["data"].as_array() {
                        for ioc in iocs {
                            let ioc_type = ioc["ioc_type"].as_str()
                            ...
                            if !ioc_value.is_empty() {
                                let _ = store.insert_finding(&NewFinding {
                                    ...
                                    description: Some(format!("Case: {} — {}", case_name, ioc_desc)),
    ...
    tracing::info!("DFIR-IRIS: Sync done — {} cases, {} IOCs, {} findings",
        result.cases_imported, result.iocs_imported, result.findings_created);
    result
}
