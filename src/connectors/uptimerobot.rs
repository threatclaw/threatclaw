...
pub async fn sync_uptimerobot(
    store: &dyn Database,
    config: &UptimeRobotConfig,
) -> UptimeRobotSyncResult {
        ...
        .await
    {
        Ok(r) => r,
        Err(e) => {
            result.errors.push(format!("UptimeRobot request: {}", e));
            return result;
        }
    };
    ...
    if !resp.status().is_success() {
        result
            .errors
            .push(format!("UptimeRobot HTTP {}", resp.status()));
        return result;
    }
    ...
    let body: serde_json::Value = match resp.json().await {
        Ok(b) => b,
        Err(e) => {
            result.errors.push(format!("UptimeRobot parse: {}", e));
            return result;
        }
    };
    ...
    if body["stat"].as_str() != Some("ok") {
        ...
        result
            .errors
            .push(format!("UptimeRobot API error: {}", msg));
        return result;
    }
    ...
    for monitor in monitors {
        ...
        match status {
            8 | 9 => {
                ...
                if let Err(e) = store
                    .insert_sigma_alert("uptimerobot-down", level, &title, "", None, None)
                    .await
                {
        ...
        if let Some(ssl_expiry) = monitor["ssl"]
            .as_object()
