...
pub async fn sync_authentik(store: &dyn Database, config: &AuthentikConfig) -> AuthentikSyncResult {
    ...
    // Fetch events
    let events_url = format!("{}/api/v3/events/events/?ordering=-created&page_size={}", url, config.max_events);
    let resp = match client.get(&events_url)
        .header("Authorization", format!("Bearer {}", config.token))
        .send().await
    {
        Ok(r) => r,
        Err(e) => { result.errors.push(format!("Events request: {}", e)); return result; }
    };

    if !resp.status().is_success() {
        let status = resp.status();
        let text = resp.text().await.unwrap_or_default();
        result.errors.push(format!("Events HTTP {}: {}", status, &text[..text.len().min(200)]));
        return result;
    }

    let data: serde_json::Value = match resp.json().await {
        Ok(d) => d,
        Err(e) => { result.errors.push(format!("Parse events: {}", e)); return result; }
    };
        ...
        match action {
            ...
            "model_created" | "model_updated" | "model_deleted" => {
                ...
                if model_name == "user" || model_name == "group" || model_name == "token" {
                    let severity = if action == "model_deleted" { "MEDIUM" } else { "LOW" };
                    let _ = store.insert_finding(&NewFinding {
                        skill_id: "skill-authentik".into(),
    ...
    tracing::info!("AUTHENTIK: Sync done — {} events, {} findings", result.events_imported, result.findings_created);
    result
}
