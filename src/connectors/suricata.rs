...
pub async fn sync_suricata(store: &dyn Database, config: &SuricataConfig) -> SuricataSyncResult {
    ...
    if let Err(e) = validate_log_path(&config.eve_json_path) {
        result
            .errors
            .push(format!("Suricata path validation failed: {}", e));
        return result;
    }
    ...
    let content = match std::fs::read_to_string(path) {
        Ok(c) => c,
        Err(e) => {
            result.errors.push(format!("Read eve.json: {}", e));
            return result;
        }
    };
    ...
    for line in content.lines() {
        ...
        match event_type {
            "alert" => {
                let signature = event["alert"]["signature"]
                    .as_str()
            ...
            "flow" => {
                ...
                {
                    let title = format!(
                        "Suricata: large flow {} → {} ({:.1} MB)",
                        src_ip,
                        dest_ip,
                        bytes_toserver as f64 / 1_000_000.0
                    );
                    let _ = store
                        .insert_sigma_alert(
                            "suricata-large-flow",
