...
static RATE_LIMITS: std::sync::LazyLock<
    Mutex<HashMap<String, (u32, chrono::DateTime<chrono::Utc>)>>,
...
pub async fn process_webhook(store: &dyn Database, source: &str, token: &str, body: &[u8]) -> u32 {
    ...
    if body.len() > max_size {
        tracing::warn!(
            "WEBHOOK: body too large for source {} ({}b)",
            source,
            body.len()
        );
        return 0;
...
pub async fn generate_token(store: &dyn Database, source: &str) -> Result<String, String> {
    ...
    let key = format!("webhook_token_{}", source);
    store
        .set_setting("webhook_ingest", &key, &serde_json::json!(token))
        .await
    ...
    for entry in entries {
        ...
        match tag.as_str() {
            ...
            "zeek.files" => {
                ...
                if let Some(hash_val) = matched_hash {
                    ...
                    tracing::warn!(
                        ...
                        filename,
                        &hash_val[..std::cmp::min(12, hash_val.len())],
                        src,
                        dst,
                        source_proto
                    );
                    ...
                    if let Some(vt_key) = vt_api_key {
                        ...
                        tokio::spawn(async move {
                            match tokio::time::timeout(
                                ...
                                crate::enrichment::virustotal_lookup::lookup_hash(
                                    &vt_hash, &vt_key,
                                ),
                            )
                            .await
                            {
                                ...
                                Ok(Ok(_)) => {
                                    tracing::debug!(
                                        "NDR-FILE-VT: {} clean on VirusTotal",
                                        vt_filename
                                    );
                                }
                                Ok(Err(e)) => {
                                    tracing::debug!(
                                        "NDR-FILE-VT: lookup failed for {}: {}",
                                        vt_hash,
                                        e
                                    );
                                }
            ...
            "zeek.notice" | "zeek.notice_log" => {
                ...
                let src = entry["id.orig_h"]
                    .as_str()
                    .or_else(|| entry["src"].as_str())
                    .unwrap_or(hostname);
                let dst = entry["id.resp_h"]
                    .as_str()
                ...
                let (rule_id, level, _mitre) = match note_type {
                    ...
                    n if n.starts_with("ATTACK::") => ("bzar-generic", "high", "T1059"),
                    "LongConnection::found" => ("zeek-long-conn-notice", "medium", "T1071.001"),
                    ...
                    n if n.contains("DNS") => ("zeek-dns-anomaly", "medium", "T1071.004"),
...
fn detect_zeek_log_type(entry: &serde_json::Value) -> String {
    ...
    if entry.get("method").is_some()
        ...
        && entry.get("status_code").is_some()
    {
        return "zeek.http".into();
    }
    if entry.get("auth_success").is_some()
        || entry.get("client").is_some()
            && entry.get("server").is_some()
            && entry.get("cipher_alg").is_some()
    {
        return "zeek.ssh".into();
    }
    if entry.get("fuid").is_some() && entry.get("mime_type").is_some() {
        return "zeek.files".into();
    }
    if entry.get("conn_state").is_some()
        || entry.get("duration").is_some() && entry.get("orig_bytes").is_some()
    {
        return "zeek.conn".into();
    }
    if entry.get("note").is_some() && entry.get("msg").is_some() {
        return "zeek.notice".into();
    }
    // Fallback
    "zeek.unknown".into()
}
...
async fn parse_suricata(store: &dyn Database, json: &serde_json::Value) -> u32 {
    ...
    for entry in entries {
        ...
        if event_type == "alert" {
            let signature = entry["alert"]["signature"]
                .as_str()
        ...
        if event_type == "flow" {
            ...
            if bytes_out > 50_000_000 && !crate::agent::ip_classifier::is_non_routable(dest_ip) {
                let title = format!(
                    "Suricata: Large outbound flow to {} ({:.1} MB)",
                    dest_ip,
...
async fn parse_cloudflare(store: &dyn Database, json: &serde_json::Value) -> u32 {
    ...
    for event in &events {
        let action = event["Action"]
            ...
            .unwrap_or("");
        let client_ip = event["ClientIP"]
            .as_str()
            .or_else(|| event["clientIP"].as_str())
            .unwrap_or("unknown");
        let path = event["ClientRequestPath"]
            .as_str()
        ...
        if action == "block" || action == "drop" || action == "challenge" {
            ...
            if store
                .insert_sigma_alert("cf-webhook", "medium", &title, "", Some(client_ip), None)
                .await
                .is_ok()
            {
                count += 1;
...
async fn parse_crowdsec(store: &dyn Database, json: &serde_json::Value) -> u32 {
    ...
    for decision in &decisions {
        ...
        if !value.is_empty() {
            ...
            if store
                .insert_sigma_alert("cs-webhook", "high", &title, "", Some(value), None)
                .await
                .is_ok()
            {
                count += 1;
...
async fn parse_fail2ban(store: &dyn Database, json: &serde_json::Value) -> u32 {
    ...
    if store
        .insert_sigma_alert("f2b-webhook", level, &title, "", Some(ip), None)
        .await
        .is_ok()
    {
        1
...
async fn parse_uptimerobot(store: &dyn Database, json: &serde_json::Value) -> u32 {
    ...
    let (title, level) = match alert_type {
        ...
        3 => (
            format!("SSL expiry warning: {} ({})", monitor_name, monitor_url),
            "medium",
        ),
        _ => (
            format!("UptimeRobot alert: {} — {}", monitor_name, alert_details),
            "medium",
        ),
    };

    if store
        .insert_sigma_alert("ur-webhook", level, &title, "", None, None)
        .await
        .is_ok()
    {
        1
...
async fn parse_uptime_kuma(store: &dyn Database, json: &serde_json::Value) -> u32 {
    ...
    let (title, level) = match heartbeat_status {
        0 => (
            format!(
                "{} DOWN — {} ({})",
                monitor_name, heartbeat_msg, monitor_url
            ),
            "high",
        ),
    ...
    if store
        .insert_sigma_alert("kuma-webhook", level, &title, "", None, None)
        .await
        .is_ok()
    {
        1
...
async fn parse_graylog(store: &dyn Database, json: &serde_json::Value) -> u32 {
    ...
    let title = json["check_result"]["result_description"]
        .as_str()
        .or_else(|| json["event_definition_title"].as_str())
        .unwrap_or("Graylog alert");

    let _message = json["check_result"]["matching_messages"]
        .as_array()
        .and_then(|a| a.first())
    ...
    let source_ip = json["check_result"]["matching_messages"]
        .as_array()
        .and_then(|a| a.first())
        .and_then(|m| m["source"].as_str());

    if store
        .insert_sigma_alert("graylog-webhook", "medium", title, "", source_ip, None)
        .await
        .is_ok()
    {
        1
...
async fn parse_strelka(store: &dyn Database, json: &serde_json::Value) -> u32 {
    ...
    for result in &results {
        ...
        if entropy > 7.5 {
            threats.push(format!(
                "Entropie très élevée: {:.2} (packed/chiffré)",
                entropy
            ));
            if severity != "CRITICAL" {
                severity = "HIGH";
    ...
    if findings > 0 {
        tracing::warn!(
            "STRELKA: {} malicious files detected from {} results",
            findings,
            results.len()
        );
    }
