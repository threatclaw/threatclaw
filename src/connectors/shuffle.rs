...
pub async fn sync_shuffle(store: &dyn Database, config: &ShuffleConfig) -> ShuffleSyncResult {
    ...
    let resp = match client
        .get(format!("{}/api/v1/workflows", url))
        ...
        .await
    {
        Ok(r) => r,
        Err(e) => {
            result.errors.push(format!("Workflows request: {}", e));
            return result;
        }
    };
    ...
    let workflows: Vec<serde_json::Value> = match resp.json().await {
        Ok(w) => w,
        Err(e) => {
            result.errors.push(format!("Parse workflows: {}", e));
            return result;
        }
    };
    ...
    for workflow in &workflows {
        ...
        let exec_resp = match client
            .get(format!("{}/api/v1/workflows/{}/executions", url, wf_id))
            .header("Authorization", format!("Bearer {}", config.api_key))
            .send()
            .await
        {
    ...
    tracing::info!(
        "SHUFFLE: Sync done — {} workflows, {} failures, {} findings",
