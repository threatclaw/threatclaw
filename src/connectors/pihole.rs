...
pub async fn sync_pihole(store: &dyn Database, config: &PiholeConfig) -> PiholeSyncResult {
        ...
        .build()
    {
        Ok(c) => c,
        Err(e) => {
            result.errors.push(format!("Pi-hole HTTP client: {}", e));
            return result;
        }
    };
        ...
        .await
    {
        Ok(r) => r,
        Err(e) => {
            result.errors.push(format!("Pi-hole auth: {}", e));
            return result;
        }
    };
    ...
    if !auth_resp.status().is_success() {
        result
            .errors
            .push(format!("Pi-hole auth HTTP {}", auth_resp.status()));
        return result;
    }
    ...
    let auth_body: serde_json::Value = match auth_resp.json().await {
        Ok(b) => b,
        Err(e) => {
            result.errors.push(format!("Pi-hole auth parse: {}", e));
            return result;
        }
    };
    ...
    // Get recent queries
    let queries_resp = match client
        .get(&format!("{}/api/queries?length=200", base))
        .header("sid", session)
        .timeout(std::time::Duration::from_secs(15))
        .send()
        .await
    {
        Ok(r) => r,
        Err(e) => {
            result.errors.push(format!("Pi-hole queries: {}", e));
            return result;
        }
    };
    ...
    if !queries_resp.status().is_success() {
        result
            .errors
            .push(format!("Pi-hole queries HTTP {}", queries_resp.status()));
        return result;
    }
    ...
    let body: serde_json::Value = match queries_resp.json().await {
        Ok(b) => b,
        Err(e) => {
            result.errors.push(format!("Pi-hole parse: {}", e));
            return result;
        }
    };
    ...
    if let Some(arr) = queries {
        for q in arr {
            ...
            if !client_ip.is_empty() {
                clients.insert(client_ip.to_string());
            }
    ...
    let mut client_blocked_count: std::collections::HashMap<String, u32> =
            ...
            if store
                .insert_finding(&NewFinding {
                    ...
                    description: Some(format!(
                        "Client {} has {} blocked DNS queries in recent history",
                        ip, count
                    )),
