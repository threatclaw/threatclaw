...
pub async fn sync_elastic_siem(
    store: &dyn Database,
    config: &ElasticSiemConfig,
) -> ElasticSiemSyncResult {
    ...
    if let (Some(id), Some(secret)) = (&config.api_key_id, &config.api_key_secret) {
        ...
        result
            .errors
            .push("No auth configured: set api_key_id+api_key_secret or username+password".into());
        return result;
    }
    ...
    tracing::info!(
        "ELASTIC: Sync done — {} alerts, {} findings",
