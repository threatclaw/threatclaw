...
#[derive(Parser, Debug)]
#[command(name = "threatclaw")]
#[command(
    about = "Secure personal AI assistant that protects your data and expands its capabilities"
)]
#[command(
    long_about = "ThreatClaw is a secure AI assistant. Use 'threatclaw <subcommand> --help' for details.\nExamples:\n  threatclaw run  # Start the agent\n  threatclaw config list  # List configs"
)]
#[command(version)]
...
pub enum Command {
    ...
    #[command(
        ...
        long_about = "Starts the ThreatClaw agent in default mode.\nExample: threatclaw run"
    )]
    Run,
    ...
    #[command(
        ...
        long_about = "Guides through initial configuration.\nExamples:\n  threatclaw onboard --skip-auth  # Skip auth step\n  threatclaw onboard --channels-only  # Reconfigure channels\n  threatclaw onboard --provider-only  # Change LLM provider and model"
    )]
    Onboard {
    ...
    #[command(
        ...
        long_about = "List configured messaging channels.\nExamples:\n  threatclaw channels list\n  threatclaw channels list --verbose\n  threatclaw channels list --json"
    )]
    ...
    #[command(
        ...
        long_about = "List, create, edit, enable/disable, delete, and view history of routines.\nExamples:\n  threatclaw routines list\n  threatclaw routines create --name daily-digest --schedule '0 0 9 * * *' --prompt 'Summarize today'"
    )]
    ...
    #[command(
        ...
        long_about = "Add, auth, list, or test MCP servers.\nExample: threatclaw mcp add notion https://mcp.notion.com"
    )]
    Mcp(Box<McpCommand>),
    ...
    #[command(
        ...
        long_about = "Approve or manage pairing requests.\nExamples:\n  threatclaw pairing list telegram\n  threatclaw pairing approve telegram ABC12345"
    )]
    ...
    #[command(
        subcommand,
        about = "Manage skills",
        long_about = "List, search, and inspect SKILL.md-based skills.\nExamples:\n  threatclaw skills list\n  threatclaw skills search 'writing'\n  threatclaw skills info my-skill"
    )]
    ...
    #[command(
        ...
        long_about = "Generates shell completion scripts.\nExample: threatclaw completion --shell bash > threatclaw.bash"
    )]
...
-> anyhow::Result<Arc<dyn crate::secrets::SecretsStore + Send + Sync>> {
    ...
    let master_key = config.secrets.master_key().ok_or_else(|| {
        anyhow::anyhow!(
            "SECRETS_MASTER_KEY not set. Run 'threatclaw onboard' first or set it in .env"
        )
    })?;
