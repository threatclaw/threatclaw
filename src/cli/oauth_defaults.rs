...
//!
//! - **Compile time**: Set THREATCLAW_GOOGLE_CLIENT_ID / THREATCLAW_GOOGLE_CLIENT_SECRET
...
const GOOGLE_CLIENT_ID: &str = match option_env!("THREATCLAW_GOOGLE_CLIENT_ID") {
    Some(v) => v,
    None => "564604149681-efo25d43rs85v0tibdepsmdv5dsrhhr0.apps.googleusercontent.com",
};
const GOOGLE_CLIENT_SECRET: &str = match option_env!("THREATCLAW_GOOGLE_CLIENT_SECRET") {
    Some(v) => v,
...
///
/// This is the case when `THREATCLAW_OAUTH_CALLBACK_URL` is set to a non-loopback
/// URL, meaning the user's browser will redirect to a hosted gateway rather than
/// localhost.
pub fn use_gateway_callback() -> bool {
    std::env::var("THREATCLAW_OAUTH_CALLBACK_URL")
        .ok()
        .filter(|v| !v.is_empty())
        .map(|raw| {
            url::Url::parse(&raw)
...
///
/// Returns the nonce unchanged when `THREATCLAW_INSTANCE_NAME` is not set
/// (local/non-platform mode).
pub fn build_platform_state(nonce: &str) -> String {
    let instance = std::env::var("THREATCLAW_INSTANCE_NAME")
...
mod tests {
    ...
    fn test_callback_host_env_override() {
        ...
        let original_url = std::env::var("THREATCLAW_OAUTH_CALLBACK_URL").ok();
        // SAFETY: Under ENV_MUTEX, no concurrent env access.
        unsafe {
        ...
        unsafe {
            ...
            if let Some(val) = original_url {
                std::env::set_var("THREATCLAW_OAUTH_CALLBACK_URL", val);
            }
        }
    }
    ...
    fn test_landing_html_success_contains_key_elements() {
        ...
        assert!(html.contains("ThreatClaw"));
    ...
    fn test_use_gateway_callback_false_for_localhost() {
        ...
        // SAFETY: Under ENV_MUTEX, no concurrent env access.
        unsafe {
            std::env::set_var("THREATCLAW_OAUTH_CALLBACK_URL", "http://127.0.0.1:3001");
        }
        assert!(!crate::cli::oauth_defaults::use_gateway_callback());
        unsafe {
            if let Some(val) = original {
    ...
    fn test_use_gateway_callback_false_for_empty() {
        ...
        // SAFETY: Under ENV_MUTEX, no concurrent env access.
        unsafe {
            std::env::set_var("THREATCLAW_OAUTH_CALLBACK_URL", "");
        }
        assert!(!crate::cli::oauth_defaults::use_gateway_callback());
        unsafe {
            if let Some(val) = original {
    ...
    fn test_build_platform_state_with_instance() {
        ...
        let _guard = crate::config::helpers::env_lock();
        let original = std::env::var("THREATCLAW_INSTANCE_NAME").ok();
        // SAFETY: Under ENV_MUTEX, no concurrent env access.
        unsafe {
            std::env::set_var("THREATCLAW_INSTANCE_NAME", "kind-deer");
        }
        assert_eq!(build_platform_state("abc123"), "kind-deer:abc123");
        unsafe {
            if let Some(val) = original {
                std::env::set_var("THREATCLAW_INSTANCE_NAME", val);
            } else {
                std::env::remove_var("THREATCLAW_INSTANCE_NAME");
            }
        }
    }
    ...
    fn test_build_platform_state_with_openclaw_instance() {
        ...
        let _guard = crate::config::helpers::env_lock();
        let original_ic = std::env::var("THREATCLAW_INSTANCE_NAME").ok();
        let original_oc = std::env::var("OPENCLAW_INSTANCE_NAME").ok();
        // SAFETY: Under ENV_MUTEX, no concurrent env access.
        unsafe {
            std::env::remove_var("THREATCLAW_INSTANCE_NAME");
