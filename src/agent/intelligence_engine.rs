...
fn extract_text_from_l2_json(v: &serde_json::Value) -> String {
    ...
    for key in &[
        ...
        "2_mitre_attck_techniques",
        "techniques",
    ] {
        if let Some(techs) = root.get(key).and_then(|v| v.as_array()) {
            let tech_strs: Vec<String> = techs
                .iter()
                .filter_map(|t| {
        ...
        if let Some(ioc) = root.get(key) {
            if let Some(arr) = ioc.as_array() {
                let strs: Vec<String> = arr
                    ...
                    .collect();
                if !strs.is_empty() {
                    parts.push(format!("IOC : {}", strs.join(", ")));
                    break;
                }
                ...
                for (k, val) in obj {
                    if let Some(s) = val.as_str() {
                        if !s.is_empty() && s != "null" {
                            ioc_parts.push(format!("{}: {}", k, s));
                        }
                    } else if let Some(arr) = val.as_array() {
                        let vals: Vec<&str> = arr.iter().filter_map(|v| v.as_str()).collect();
                        if !vals.is_empty() {
                            ioc_parts.push(format!("{}: {}", k, vals.join(", ")));
                        }
                    } else if let Some(b) = val.as_bool() {
                        if b {
                            ioc_parts.push(k.clone());
                        }
                    }
                }
                if !ioc_parts.is_empty() {
                    parts.push(format!("IOC : {}", ioc_parts.join(", ")));
                    break;
                }
            }
        }
    ...
    for key in &[
        ...
        "4_actions_recommandees_prioritaires",
...
///
/// ThreatClaw escalates an event to a full incident **only** when its asset
/// resolves to something the customer asked us to monitor. External IPs that
/// appear in logs (Internet scanners, blocked traffic sources, threat actors)
/// are observations, not assets — they belong to a separate audit track and
/// must not pollute the RSSI's incident queue.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum AssetScope {
    /// Asset is in the customer inventory or matches a declared internal
    /// network. `reason` describes which rule fired (used for logs).
    Monitored(&'static str),
    /// Asset string parses as a public IPv4 that is not in the inventory.
    /// Don't escalate — this is external traffic.
    External,
    /// Asset string is neither a recognised name in the inventory nor a
    /// parseable IP. Treated as external (safer default than escalating).
    Unknown,
}
...
/// Test whether an IPv4 falls inside a CIDR block specified as `"a.b.c.d/N"`.
/// Returns `false` for malformed input — callers shouldn't propagate config errors.
fn cidr_contains_ipv4(cidr: &str, ip: std::net::Ipv4Addr) -> bool {
    let (net_str, prefix_str) = match cidr.split_once('/') {
        Some(p) => p,
        None => return false,
    };
    let net = match net_str.parse::<std::net::Ipv4Addr>() {
        Ok(n) => n,
        Err(_) => return false,
    };
...
/// Classify an asset string against the customer inventory.
///
/// Resolution order:
///   1. Exact match in the `assets` table (id / hostname / FQDN / ip_addresses)
///   2. IPv4 falling inside any declared `internal_networks` CIDR
///   3. IPv4 in RFC1918 — kept as a safety fallback so the gate never starves
///      a fresh install whose operator hasn't filled the network table yet
///   4. Otherwise — External (public IPv4 not in inventory) or Unknown
///      (string is not an IP and not in inventory)
///
/// `internal_networks` is passed in pre-fetched so the IE can call this
/// helper many times per cycle without hammering the database.
pub async fn classify_asset(
    store: &dyn crate::db::Database,
    asset: &str,
    internal_networks: &[crate::db::threatclaw_store::InternalNetwork],
) -> AssetScope {
...
async fn build_dossier_from_situation(
    ...
    let mut asset_findings: Vec<DossierFinding> = all_findings
        .iter()
...
pub async fn run_intelligence_cycle(store: Arc<dyn Database>) -> SecuritySituation {
    ...
    for entry in asset_map.values_mut() {
        ...
        entry.summary = if entry.summary.is_empty() {
            build_asset_summary(entry)
        } else {
            entry.summary.clone()
        };
    }
    ...
    for threat in &log_scan_results {
        let dedup_key = format!(
            "{}|{}|{}",
            threat.title,
            threat.asset.as_deref().unwrap_or(""),
            threat.source
        );
        if !finding_dedup.insert(dedup_key) {
            continue;
        }
        let _ = store
    ...
    if !log_scan_results.is_empty() {
        tracing::info!(
            "INTELLIGENCE: {} threat(s) detected from log scan",
            log_scan_results.len()
        );
    ...
    // ── 5j. BLOOM FILTER — uses shared recent_logs (See ADR-030)
    {
        ...
        for log in &recent_logs {
            let detected = crate::agent::ioc_bloom::check_log(
                &log.data,
                log.hostname.as_deref(),
                store.as_ref(),
            )
            .await;
            for ioc in &detected {
                ...
                let _ = store
                    .insert_finding(&crate::db::threatclaw_store::NewFinding {
                        ...
                        title: format!(
                            "{} malveillant détecté: {}",
                            ioc.ioc_type.to_uppercase(),
                            ioc.ioc_value
                        ),
                        description: Some(format!(
                            "IoC de type {} identifié par Bloom filter, confirmé par {}",
                            ioc.ioc_type, ioc.source
                        )),
    ...
    let ndr_total = ja3_result.matches_found
        + beacon_result.beacons_detected
        + tls_result.anomalies_found
        + dns_tunnel_result.tunnels_detected
        + sni_result.suspects_found
        + ransomware_result.suspects_found;
    if ndr_total > 0 {
        tracing::info!(
            "NDR: JA3={} beacons={} TLS={} DNS-tunnel={} SNI={} ransomware={}",
            ja3_result.matches_found,
            beacon_result.beacons_detected,
            tls_result.anomalies_found,
            dns_tunnel_result.tunnels_detected,
            sni_result.suspects_found,
        ...
        if hour == 3 && minute < 5 {
            ...
            if retroact_result.candidates_checked > 0 {
                tracing::info!(
                    ...
                    retroact_result.candidates_checked,
                    retroact_result.upgraded,
                    retroact_result.cleared
                );
            }
        }
    }
    ...
    // ── 5n. CTI FEED SYNC — unified threat intel (every 6 hours) ──
    {
        ...
        if hour % 6 == 0 && minute < 5 {
            ...
            tracing::info!(
                ...
                cti_result.sources_synced,
                cti_result.iocs_new,
                cti_result.errors.len()
            );
        }
    }
    ...
    for f in &findings {
            ...
            .and_then(|v| v.as_str())
        {
            ...
            let epss_cached =
                crate::agent::production_safeguards::get_cached_ioc(store.as_ref(), "epss", cve)
                    .await;
            let epss_result = if let Some(cached) = epss_cached {
                ...
                {
                    Ok(Ok(Some(ref epss))) => {
                        crate::agent::production_safeguards::cache_ioc(
                            store.as_ref(),
                            "epss",
                            cve,
                            &json!({"epss": epss.epss}),
            ...
            if let Some(epss_val) = epss_result {
                ...
                if pct > 50.0 {
                    enrichment_lines.push(format!(
                        "EPSS {}: {:.0}% probabilité exploitation 30j",
                        cve, pct
                    ));
                }
                ...
                if let Some(entry) = asset_map.get_mut(asset_name) {
                    if epss_val > 0.9 {
                        entry.score = (entry.score + 20.0).min(100.0);
                    }
                }
            }
            ...
            {
                enrichment_lines.push(format!(
                    "CISA KEV: {} activement exploitée — action requise avant {}",
                    cve, kev.due_date
                ));
                let asset_name = f.asset.as_deref().unwrap_or("unknown");
                if let Some(entry) = asset_map.get_mut(asset_name) {
    ...
    for a in &alerts {
        ...
        if let Some(ref raw_ip) = a.source_ip {
            ...
            if rate_limiter.ip_already_seen(ip_str) {
                continue;
            }

            // GreyNoise — cache first
            let gn_cached = crate::agent::production_safeguards::get_cached_ioc(
                store.as_ref(),
                "greynoise",
                ip_str,
            )
            ...
            if let Some(cached) = gn_cached {
                ...
                let label = if classification == "malicious" {
                    "attaque ciblée"
                } else if cached["noise"].as_bool() == Some(true) {
                    "scanner de masse"
                } else {
                    "inconnu"
                };
                ...
                .await
                {
                    Ok(Ok(gn)) => {
                        ...
                        let label = if gn.classification == "malicious" {
                            "attaque ciblée"
                        } else if gn.noise {
                            "scanner de masse"
                        } else {
                        ...
                        enrichment_lines.push(format!(
                            "GreyNoise {}: {} ({})",
                            ip_str, gn.classification, label
                        ));
                    }
            ...
            let geo_cached = crate::agent::production_safeguards::get_cached_ioc(
                store.as_ref(),
                "ipinfo",
                ip_str,
            )
                ...
                .await
                {
                    Ok(Ok(geo)) => {
                        crate::agent::production_safeguards::cache_ioc(
                            ...
                            "ipinfo",
                            ip_str,
                            &json!({"country": geo.country, "org": geo.org}),
                        )
                        .await;
                        enrichment_lines.push(format!(
                            "Origine: {}",
                            crate::enrichment::ipinfo::format_for_context(&geo)
                        ));
                    }
    ...
    let alert_message = if notification_level >= NotificationLevel::Alert {
        Some(build_enriched_alert_message(
            &assets,
            &alerts,
            &enrichment_lines,
            &findings,
    ...
    let _ = store
        ...
        .await;
    let _ = store
        .record_metric("security_score", global_score, &json!({}))
        .await;
...
pub async fn reinvestigate_incident(
    ...
    let current_verdict = incident["verdict"]
        .as_str()
    ...
    tracing::info!(
        "REINVESTIGATE: Incident #{} ({}) — calling L2",
        incident_id,
        asset
    );
    ...
    let parsed = serde_json::from_str::<serde_json::Value>(&l2_clean)
        ...
        .ok_or_else(|| {
            format!(
                "L2 returned non-JSON output: {}",
                &l2_clean[..l2_clean.len().min(200)]
            )
        })?;
    ...
    let _ = store
        .add_incident_note(
            incident_id,
            &format!(
                "Investigation relancée — {} MITRE, {} actions, {} IOCs",
                parsed_mitre.len(),
                parsed_actions.len(),
                parsed_iocs.len()
            ),
    ...
    if let Ok(Some(last)) = store.get_setting("_system", "last_mitre_retry").await {
        if let Some(ts) = last.as_str() {
            if let Ok(last_dt) = chrono::DateTime::parse_from_rfc3339(ts) {
                let elapsed =
                    (chrono::Utc::now() - last_dt.with_timezone(&chrono::Utc)).num_seconds();
                if elapsed < 6 * 3600 {
                    return;
                }
            }
...
async fn check_daily_cleanup(store: &dyn Database) {
    ...
    if cur_h != 3 {
        return;
    } // Run between 03:00 and 03:10 UTC
    ...
    match store.purge_old_archived(60).await {
        Ok((inc, al)) => tracing::info!(
            "CLEANUP: Purged {} archived incidents, {} archived alerts",
            inc,
            al
        ),
    ...
    let _ = store
        .set_setting("_system", "last_daily_cleanup", &serde_json::json!(today))
        .await;
}
...
async fn check_unresolved_reminders(store: &dyn Database) {
    ...
    for (severity, remind_secs) in thresholds {
        ...
        let incidents = store
            .list_incidents(Some("open"), 50, 0)
            .await
            .unwrap_or_default();
        for inc in &incidents {
            let inc_severity = inc["severity"].as_str().unwrap_or("");
            if inc_severity != severity {
                continue;
            }
            ...
            if hitl_status == "responded" || hitl_status == "approved" {
                continue;
            }
            ...
            if should_remind {
                tracing::info!(
                    "REMIND: Incident #{} ({} {}) unresolved after {}h — re-notifying",
                    inc_id,
                    asset,
                    severity,
                    remind_secs / 3600
                ...
                let remind_text = format!(
                    "⏰ RAPPEL — Incident #{} ({}) non traité depuis {}h",
                    inc_id,
                    asset,
                    remind_secs / 3600
                );
...
async fn check_daily_digest(store: &dyn Database, situation: &SecuritySituation) {
    ...
    {
        let tz = profile["business_hours"]["timezone"]
            .as_str()
            .unwrap_or("Europe/Paris");
        if tz.contains("Paris") || tz.contains("Berlin") || tz.contains("CET") {
            2i64
        } else {
            0
        }
    ...
    let local_h =
        ((now.format("%H").to_string().parse::<i64>().unwrap_or(12) + tz_offset) % 24) as u32;
    ...
    if local_min < target_min || local_min > target_min + 10 {
        return;
    }
    ...
    // Build digest
    let open_incidents = store
        .list_incidents(Some("open"), 10, 0)
        .await
        .unwrap_or_default();
    let open_count = open_incidents.len();
    let critical_count = open_incidents
        .iter()
    ...
    let digest = format!(
        ...
        if open_count == 0 {
            "Aucun incident en cours. Bonne journée ! 👍".into()
        } else {
            let top: Vec<String> = open_incidents
                .iter()
                .take(3)
                .map(|i| {
                    format!(
    ...
    let _ = store
        .set_setting("_system", digest_key, &serde_json::json!(today))
        .await;
...
fn decide_notification_level(
    ...
    if assets
        .values()
        .any(|a| a.has_kill_chain && a.has_known_exploit)
    {
    ...
    if assets
        .values()
        .any(|a| a.findings_critical > 0 && a.active_alerts > 0)
    {
        return NotificationLevel::Critical;
    }
    ...
    if !assets.is_empty() {
        ...
        for asset in assets.iter().take(5) {
            ...
            msg.push_str(&format!(
                ...
                asset.asset,
                level,
                asset.findings_critical
                    + asset.findings_high
                    + asset.findings_medium
                    + asset.findings_low,
                asset.active_alerts,
...
fn build_alert_message(
    ...
    let empty_intel = GraphIntelSummary {
        lateral_detections: 0,
        lateral_summary: String::new(),
        campaigns_count: 0,
        campaigns_summary: String::new(),
        actors_count: 0,
        actors_summary: String::new(),
...
fn build_enriched_alert_message(
    ...
    let en = lang == "en";
    let critical_assets: Vec<&AssetSituation> = assets.iter().filter(|a| a.score >= 30.0).collect();

    let is_critical = critical_assets
        .iter()
        .any(|a| a.has_kill_chain || a.has_known_exploit || a.findings_critical > 0);
    ...
    // ── LLM Analysis ──
    let ai_finding = findings.iter().rev().find(|f| {
        f.source.as_deref() == Some("threatclaw-l1") || f.source.as_deref() == Some("threatclaw-l2")
    });
    if let Some(f) = ai_finding {
        if let Some(ref desc) = f.description {
            let analysis = desc
                ...
                .trim()
                .to_string();
            let source = if f.source.as_deref() == Some("threatclaw-l2") {
                "L2 Forensique"
            } else {
                "L1 Triage"
            };
    ...
    if !critical_assets.is_empty() {
        ...
        for asset in critical_assets.iter().take(3) {
            ...
            if asset.findings_critical > 0 {
                flags.push(if en { "critical vuln" } else { "vuln critique" });
            }
    ...
    if graph_intel.lateral_detections > 0 {
        ...
        intel_lines.push(format!(
            "{} : {}",
            label,
            graph_intel.lateral_summary.lines().next().unwrap_or("")
        ));
    }
    ...
    if graph_intel.campaigns_count > 0 {
        ...
        intel_lines.push(format!(
            "{} : {}",
            label,
            graph_intel.campaigns_summary.lines().next().unwrap_or("")
        ));
    }
    ...
    if !enrichment.is_empty() {
        ...
        let deduped: Vec<&String> = enrichment
            .iter()
    ...
    if msg.len() > 4000 {
        msg.truncate(3900);
        msg.push_str(if en {
            "\n...\n_[see dashboard /intelligence]_"
        } else {
...
pub fn spawn_intelligence_ticker(
    ...
    tokio::spawn(async move {
        tracing::info!(
            "INTELLIGENCE: Engine started — cycle every {}s",
            interval.as_secs()
        );
        ...
        loop {
            ...
            let has_kill_chain = situation
                .assets
                .iter()
                .any(|a| a.has_kill_chain || a.has_active_attack);
            ...
            tracing::debug!(
                "INTELLIGENCE: Next cycle in {}s (score={:.0})",
            ...
            if situation.notification_level >= NotificationLevel::Alert {
                ...
                if let Some(worst_asset) = situation.assets.iter().max_by(|a, b| {
                    a.score
                        .partial_cmp(&b.score)
                        .unwrap_or(std::cmp::Ordering::Equal)
                }) {
                    // ── Inventory gate ────────────────────────────────────
                    // Only escalate when the worst asset is in the customer
                    // inventory (or in a declared internal network / RFC1918
                    // fallback). External IPs that show up in firewall, IDS or
                    // ML logs are observations, not assets to protect — we
                    // keep the underlying findings for forensic but don't
                    // create an incident the RSSI has to triage.
                    let internal_networks =
                        store.list_internal_networks().await.unwrap_or_default();
                    let scope =
                        classify_asset(store.as_ref(), &worst_asset.asset, &internal_networks)
                            .await;
                    if !matches!(scope, AssetScope::Monitored(_)) {
                        tracing::info!(
                            asset = %worst_asset.asset,
                            scope = ?scope,
                            "INTELLIGENCE: skipping — asset is not in the inventory \
                             (external observation, no incident created)"
                        );
                        // Don't continue to incident creation. Note that we
                        // can't `continue` here because we're inside an
                        // `if let Some(...)` arm — fall through past the
                        // entire investigation block instead.
                    } else {
                        ...
                        if already_investigated {
                            tracing::debug!(
                                "INTELLIGENCE: Skipping investigation for {} — same pattern already investigated",
                                worst_asset.asset
                            );
                            ...
                            'incident_creation: {
                                    ...
                                    .await
                                {
                                    ...
                                    .await
                                    {
                                        Ok(Ok(l2_raw)) => {
                                            ...
                                            if let Some(obj) = parsed {
                                                ...
                                                if is_garbage {
                                                    tracing::warn!(
                                                        "INVESTIGATION: L2 output is low quality ({} chars, repeated blocks), using L1 only",
                                                        l2_clean.len()
                                                    );
                                                } else {
                                        ...
                                        Ok(Err(e)) => tracing::warn!(
                                            "INVESTIGATION: L2 enrichment failed for {}: {}",
                                            asset_name,
                                            e
                                        ),
                                        Err(_) => tracing::warn!(
                                            "INVESTIGATION: L2 enrichment timeout for {}",
                                            asset_name
                                        ),
                                    }
                                        }
                                    ...
                                    .await
                                    {
                                        ...
                                        tracing::debug!(
                                            "INVESTIGATION: Re-notification suppressed (no delta) for {}",
                                            asset_name
                                        );
                                    }
                                        }
...
async fn propose_hitl_actions(
    ...
    for asset in &situation.assets {
        ...
        if asset.score > 50.0 && asset.active_alerts > 0 {
            ...
            proposed_actions.push((
                ...
                asset.asset.clone(),
                format!(
                    "Scanner {} — score {:.0}, {} alertes actives",
                    asset.asset, asset.score, asset.active_alerts
                ),
            ));
        }
    ...
    if let Ok(alerts) = store
        .list_alerts(Some("critical"), Some("new"), 10, 0)
        .await
    {
        for alert in &alerts {
    ...
    tracing::info!(
        "INTELLIGENCE HITL: Proposing {} actions",
...
async fn scan_logs_for_threats(
    store: Arc<dyn Database>,
    prefetched_logs: &[crate::db::threatclaw_store::LogRecord],
) -> Vec<DetectedThreat> {
...
mod tests {
    ...
    fn test_notification_levels() {
        ...
        // Empty → Silence
        assert_eq!(
            decide_notification_level(&assets, 100.0),
            NotificationLevel::Silence
        );
        ...
        assert_eq!(
            decide_notification_level(&assets, 70.0),
            NotificationLevel::Digest
        );
        ...
        assert_eq!(
            decide_notification_level(&assets, 40.0),
