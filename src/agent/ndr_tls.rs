...
pub async fn scan_tls(store: std::sync::Arc<dyn Database>, minutes_back: i64) -> TlsScanResult {
    ...
    for log in &logs {
        ...
        if dst.is_empty() || port == 0 {
            continue;
        }

        let dedup_key = format!("{}:{}", dst, port);
        if seen_dests.contains(&dedup_key) {
            continue;
        }
        ...
        let mut anomaly = TlsAnomalies {
            ...
            validation_status: log.data["validation_status"]
                .as_str()
                .unwrap_or("")
                .to_string(),
        ...
        if !validation.is_empty()
            && validation != "ok"
            && !validation.contains("self signed")
            && !validation.contains("expired")
        {
            anomaly.score += SCORE_VALIDATION_FAIL;
            anomaly
                .reasons
                .push(format!("Validation TLS échouée: {}", validation));
        }
        ...
        if anomaly.server_name.is_empty() && (port == 443 || port == 8443) {
            ...
            anomaly
                .reasons
                .push("SNI absent sur connexion HTTPS".into());
        }

        // ── Rule 5: Short certificate validity ──
        // Zeek provides not_valid_before and not_valid_after
        if let (Some(before), Some(after)) = (
            log.data["not_valid_before"]
                .as_str()
                .or(log.data["certificate.not_valid_before"].as_str()),
            log.data["not_valid_after"]
                .as_str()
                .or(log.data["certificate.not_valid_after"].as_str()),
        ) {
            if let (Ok(start), Ok(end)) = (
                ...
                if validity_days > 0 && validity_days < 30 {
                    ...
                    anomaly
                        .reasons
                        .push(format!("Validité très courte: {} jours", validity_days));
                }
            }
        }
...
async fn create_tls_finding(store: &dyn Database, anomaly: &TlsAnomalies, hostname: Option<&str>) {
    let severity = if anomaly.score >= SCORE_CRITICAL {
        "CRITICAL"
    } else {
        "HIGH"
    };
    let reasons_text = anomaly.reasons.join(", ");
    let sni = if anomaly.server_name.is_empty() {
        "absent"
    } else {
        &anomaly.server_name
    };
    ...
    let title = format!(
        ...
        if anomaly.server_name.is_empty() {
            format!("{}:{}", anomaly.dst_ip, anomaly.dst_port)
        } else {
            anomaly.server_name.clone()
        },
        anomaly.score
    );
    ...
    let description = format!(
        ...
        anomaly.dst_port,
        sni,
        if anomaly.validation_status.is_empty() {
            "inconnue"
        } else {
