...
pub async fn scan_ransomware(
    ...
    let smb_logs = store
        .query_logs(minutes_back, None, Some("zeek.smb_files"), 5000)
        .await
        .unwrap_or_default();
    ...
    let wazuh_logs = store
        .query_logs(minutes_back, None, Some("wazuh.alert"), 3000)
        .await
        .unwrap_or_default();
    ...
    for log in &wazuh_logs {
        ...
        let hostname = log.data["agent"]["name"]
            .as_str()
    ...
    let osquery_logs = store
        .query_logs(minutes_back, None, Some("osquery.file"), 3000)
        .await
        .unwrap_or_default();
    ...
    for (hostname, ops) in &host_ops {
        ...
        if ransom_ext_count > 0 {
            ...
            if ransom_ext_count > 10 {
                score += 20;
            } // Bonus for mass encryption
        }

        // 3. Ransom note filenames
        let notes: Vec<String> = ops
            .iter()
        ...
        if total >= VOLUME_THRESHOLD {
            evidence.push(format!(
                "{} opérations fichier en {} min (seuil: {})",
        ...
        if evidence.is_empty() || (evidence.len() < 2 && score < SCORE_HIGH) {
            continue;
        }

        result.suspects_found += 1;
        let severity = if score >= SCORE_CRITICAL {
            "CRITICAL"
        } else {
            "HIGH"
        };
...
fn normalize_action(action: &str) -> String {
    ...
    if lower.contains("creat") || lower.contains("added") || lower.contains("write") {
        return "create".into();
    }
