...
//! ## Architecture
//!
//! ThreatClaw runs inside a Docker container with an entrypoint loop that monitors exit codes:
//! - **Exit code 0** (clean): Reset failure counter, wait `THREATCLAW_RESTART_DELAY` (default 5s), restart
//! - **Exit code ≠ 0** (failure): Increment failure counter, exit after `THREATCLAW_MAX_FAILURES` (default 10)
//!
...
impl Tool for RestartTool {
    ...
    fn description(&self) -> &str {
        "Restart the ThreatClaw agent process. The process exits cleanly (code 0) and the \
    ...
    async fn execute(
        ...
        // Check if running inside a Docker container via THREATCLAW_IN_DOCKER env var.
        // The Docker entrypoint sets this to "true". For local development, it's unset or "false".
        // The entrypoint restart loop only works inside a Docker container (threatclaw-worker).
        let in_docker = std::env::var("THREATCLAW_IN_DOCKER")
            .map(|v| v.to_lowercase() == "true")
            .unwrap_or(false);

        tracing::debug!("[RestartTool::execute] THREATCLAW_IN_DOCKER={}", in_docker);
        ...
        if !in_docker {
            ...
            return Err(ToolError::ExecutionFailed(
                "Restart is only available when running inside the Docker container. \
                 For local development, please restart ThreatClaw manually."
                    .to_string(),
            ));
        }
        ...
        //
        // - The threatclaw-worker Docker container runs an entrypoint loop that monitors
        //   the exit code of the `threatclaw run` process:
        //   * Exit code 0 = clean restart: reset failure counter, wait THREATCLAW_RESTART_DELAY
        //     (default 5s), then restart the process
        //   * Exit code ≠ 0 = failure: increment counter, exit after THREATCLAW_MAX_FAILURES
        //     (default 10 failures)
        //
        ...
        let restart_disabled = std::env::var("THREATCLAW_DISABLE_RESTART")
            .map(|v| {
        ...
        tokio::spawn(async move {
            ...
            if !restart_disabled {
                ...
                tracing::info!(
                    "[RestartTool] Exit disabled (THREATCLAW_DISABLE_RESTART set), skipping std::process::exit(0)"
                );
            }
        });
        ...
        let msg = format!(
            "Restarting in {delay} second(s). The process will exit cleanly and the \
             entrypoint restart loop will bring ThreatClaw back online."
        );
...
mod tests {
    ...
    fn enable_docker_env() {
        unsafe {
            std::env::set_var("THREATCLAW_IN_DOCKER", "true");
        }
    }
    ...
    fn test_restart_tool_description() {
        ...
        assert!(desc.contains("ThreatClaw"));
    ...
    fn test_restart_tool_requires_docker_environment() {
        // Test that restart is rejected when not in Docker (THREATCLAW_IN_DOCKER not set or false)
        ...
        if !in_docker {
            // Simulating what the tool would do when THREATCLAW_IN_DOCKER is not set
            assert!(
                !in_docker,
                "Test environment should have THREATCLAW_IN_DOCKER unset or false"
            );
        }
    }
}
