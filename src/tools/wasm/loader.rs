...
//! loader.load_from_dir(Path::new("~/.threatclaw/tools/")).await?;
//! ```
//!
//! # Dev Mode
//!
...
///
/// Checks (in order):
/// 1. `THREATCLAW_TOOLS_SRC` env var
/// 2. `<CARGO_MANIFEST_DIR>/tools-src/` (dev builds)
fn tools_src_dir() -> PathBuf {
    if let Ok(dir) = std::env::var("THREATCLAW_TOOLS_SRC") {
...
/// needing to install them to `~/.threatclaw/tools/` first. Build artifacts
/// that are newer than installed copies take priority.
///
/// Set `THREATCLAW_TOOLS_SRC` env var to override the source directory.
