...
5. Install with `threatclaw tool install path/to/tool.wasm`
...
When running `threatclaw tool auth <tool>`:
...
Both are first-class in the extension system (`threatclaw tool install` handles both), but they have different strengths.

**WASM Tools (ThreatClaw native)**
...
- Cost: external process with full system access (no sandbox), manages own credentials, ThreatClaw can't prevent leaks
