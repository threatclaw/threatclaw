...
pub struct DatabaseConfig {
    ...
    // -- libSQL fields --
    /// Path to local libSQL database file (default: ~/.threatclaw/threatclaw.db).
...
impl DatabaseConfig {
    pub(crate) fn resolve() -> Result<Self, ConfigError> {
        ...
        // DATABASE_URL is loaded from ~/.threatclaw/.env via dotenvy early in startup.
...
/// Default libSQL database path (~/.threatclaw/threatclaw.db).
pub fn default_libsql_path() -> PathBuf {
    threatclaw_base_dir().join("threatclaw.db")
}
