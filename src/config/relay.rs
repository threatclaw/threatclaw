...
impl RelayConfig {
    ...
    fn from_env_reader(env: impl Fn(&str) -> Option<String>) -> Option<Self> {
        ...
        Some(Self {
            url,
            api_key,
            callback_url: env("THREATCLAW_OAUTH_CALLBACK_URL"),
            instance_id: env("THREATCLAW_INSTANCE_ID"),
...
mod tests {
    ...
    fn from_env_reader_loads_overrides() {
        let config = RelayConfig::from_env_reader(|key| match key {
            ...
            "THREATCLAW_OAUTH_CALLBACK_URL" => Some("https://tunnel.example.com".into()),
            "THREATCLAW_INSTANCE_ID" => Some("my-instance".into()),
