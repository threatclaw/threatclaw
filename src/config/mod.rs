//! Configuration for ThreatClaw.
//!
//! Settings are loaded with priority: env var > database > default.
//! `DATABASE_URL` lives in `~/.threatclaw/.env` (loaded via dotenvy early
...
impl Config {
    ...
    pub fn for_testing(
        ...
        Self {
            ...
            channels: ChannelsConfig {
                ...
                gateway: None,
                signal: None,
                wasm_channels_dir: std::env::temp_dir().join("threatclaw-test-channels"),
    ...
    pub async fn from_db_with_toml(
        ...
        crate::bootstrap::load_threatclaw_env();
    ...
    ///
    /// Loads both `./.env` (standard, higher priority) and `~/.threatclaw/.env`
    ...
    /// If `None`, tries the default path `~/.threatclaw/config.toml` (missing
...
pub(crate) fn resolve_owner_id(settings: &Settings) -> Result<String, ConfigError> {
    let env_owner_id = self::helpers::optional_env("THREATCLAW_OWNER_ID")?;
                ...
                .is_some_and(|value| !value.trim().is_empty()))
    {
        WARNED_EXPLICIT_DEFAULT_OWNER_ID.call_once(|| {
            tracing::warn!(
                "THREATCLAW_OWNER_ID resolved to the legacy 'default' scope explicitly; durable state will keep legacy owner behavior"
            );
        });
    }
