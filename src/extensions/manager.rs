...
fn channel_auth_instructions(
    ...
    if channel_name == TELEGRAM_CHANNEL_NAME && secret.name == "telegram_bot_token" {
        return format!(
            "{} After you submit it, ThreatClaw will show a one-time verification code. Send `/start CODE` to your bot in Telegram and ThreatClaw will finish setup automatically.",
            secret.prompt
        );
    }
...
fn telegram_verification_instructions(bot_username: Option<&str>, code: &str) -> String {
    if let Some(username) = bot_username.filter(|username| !username.trim().is_empty()) {
        return format!(
            "Send `/start {code}` to @{username} in Telegram. ThreatClaw will finish setup automatically."
        );
    }
...
impl ExtensionManager {
    ...
    /// used to build OAuth redirect URIs when `THREATCLAW_OAUTH_CALLBACK_URL` is not set.
    ...
    /// - `THREATCLAW_OAUTH_CALLBACK_URL` is set to a non-loopback URL, OR
    ...
    ///
    /// Priority:
    /// 1. `THREATCLAW_OAUTH_CALLBACK_URL` env var (via `callback_url()`)
    ...
    pub async fn remove(&self, name: &str) -> Result<String, ExtensionError> {
        ...
        match kind {
            ...
            ExtensionKind::WasmChannel => {
                ...
                Ok(format!(
                    "Removed channel '{}'. Restart ThreatClaw for the change to take effect.",
                    name
                ))
            }
            ExtensionKind::ChannelRelay => {
            ...
            crate::registry::artifacts::find_wasm_artifact(&resolved_dir, binary_name, "release")
                ...
                ExtensionError::InstallFailed(format!(
                    "'{}' requires building from source. Build artifact not found. \
                         ...
                         or use `threatclaw registry install {}`.",
                    name,
        ...
        let client_id = self
            ...
            .ok_or_else(|| {
                ...
                if auth.secret_name.to_lowercase().contains("google") {
                    msg.push_str(", or build with THREATCLAW_GOOGLE_CLIENT_ID");
                }
                msg.push('.');
                msg
            })?;
    ...
    async fn auth_channel_relay(&self, name: &str) -> Result<AuthResult, ExtensionError> {
        ...
        let user_id_uuid = std::env::var("THREATCLAW_USER_ID").unwrap_or_else(|_| {
            uuid::Uuid::new_v5(&uuid::Uuid::NAMESPACE_DNS, self.user_id.as_bytes()).to_string()
        });
...
mod tests {
        ...
        manager
            .set_test_telegram_binding_resolver(Arc::new(|_token, existing_owner_id| {
                ...
                Ok(TelegramBindingResult::Pending(VerificationChallenge {
                    ...
                    instructions:
                        "Send `/start iclaw-7qk2m9` to @test_hot_bot in Telegram. ThreatClaw will finish setup automatically."
                            .to_string(),
    ...
    // `THREATCLAW_OAUTH_CALLBACK_URL` env var, ignoring `self.tunnel_url`.
    ...
    fn make_manager_with_tunnel(tunnel_url: Option<String>) -> ExtensionManager {
        ...
        let tools = Arc::new(crate::tools::ToolRegistry::new());
        let mcp = Arc::new(McpSessionManager::new());
        let dir = std::env::temp_dir().join("threatclaw-test-gateway-mode");
    ...
    /// Clears `THREATCLAW_OAUTH_CALLBACK_URL` for the duration, restoring on drop.
    struct EnvGuard {
