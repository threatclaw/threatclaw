...
//!
//! Stores user preferences in ~/.threatclaw/settings.json.
...
pub struct Settings {
    ...
    /// Stable owner scope for this ThreatClaw instance.
    ///
...
fn default_sandbox_image() -> String {
    "threatclaw-worker:latest".to_string()
}
...
impl Settings {
    ...
    /// Get the default settings file path (~/.threatclaw/settings.json).
    pub fn default_path() -> std::path::PathBuf {
        threatclaw_base_dir().join("settings.json")
    }
    ...
    /// Default TOML config file path (~/.threatclaw/config.toml).
    pub fn default_toml_path() -> PathBuf {
        threatclaw_base_dir().join("config.toml")
    }
    ...
    pub fn save_toml(&self, path: &std::path::Path) -> Result<(), String> {
        ...
        let content = format!(
            "# ThreatClaw configuration file.\n\
             #\n\
             # Priority: env var > this file > database settings > defaults.\n\
             # Uncomment and edit values to override defaults.\n\
             # Run `threatclaw config init` to regenerate this file.\n\
             #\n\
             # Documentation: https://github.com/nearai/threatclaw\n\
             \n\
             {raw}"
        );
...
mod tests {
    ...
    fn test_get_setting() {
        ...
        assert_eq!(settings.get("agent.name"), Some("threatclaw".to_string()));
        assert_eq!(
    ...
    fn test_reset_setting() {
        ...
        assert_eq!(settings.agent.name, "threatclaw");
    }
    ...
    fn toml_header_comment_present() {
        ...
        assert!(content.starts_with("# ThreatClaw configuration file."));
    ...
    #[test]
    fn default_toml_path_under_threatclaw() {
        let path = Settings::default_toml_path();
        assert!(path.to_string_lossy().contains(".threatclaw"));
    ...
    fn wizard_recovery_step1_overrides_stale_db() {
        ...
        let prior_run = Settings {
            ...
            database_url: Some("postgres://old-host/threatclaw".to_string()),
            llm_backend: Some("anthropic".to_string()),
        ...
        let step1_settings = Settings {
            database_backend: Some("postgres".to_string()),
            database_url: Some("postgres://new-host/threatclaw".to_string()),
            ..Settings::default()
        };
        ...
        assert_eq!(
            ...
            Some("postgres://new-host/threatclaw".to_string()),
    ...
    /// Simulates `threatclaw onboard --provider-only` re-running on a fully
    ...
    fn provider_only_rerun_preserves_unrelated_settings() {
        ...
        let prior = Settings {
            onboard_completed: true,
            database_backend: Some("libsql".to_string()),
            libsql_path: Some("/home/user/.threatclaw/threatclaw.db".to_string()),
            llm_backend: Some("openai".to_string()),
            selected_model: Some("gpt-4o".to_string()),
            embeddings: EmbeddingsSettings {
                enabled: true,
    ...
    /// Simulates `threatclaw onboard --channels-only` re-running on a fully
    ...
    fn switching_db_backend_allows_fresh_connection_settings() {
        ...
        assert_eq!(
            ...
            Some("/home/user/.threatclaw/threatclaw.db")
        );
