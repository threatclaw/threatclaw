...
//! ```text
//!   ┌─────────────────────┐    ┌──────────────────────────┐
//!   │  providers.json     │    │ ~/.threatclaw/providers.json│
...
/// from `~/.threatclaw/providers.json`.
...
impl ProviderRegistry {
    ...
    ///
    /// User providers from `~/.threatclaw/providers.json` are appended,
...
fn user_providers_path() -> Option<std::path::PathBuf> {
    Some(crate::bootstrap::threatclaw_base_dir().join("providers.json"))
}
