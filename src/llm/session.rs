...
//! OAuth flow. Tokens are stored in `~/.threatclaw/session.json` and refreshed
...
pub struct SessionConfig {
    ...
    /// Path to session file (e.g., ~/.threatclaw/session.json).
...
impl SessionManager {
    ...
    /// `~/.threatclaw/.env` for survival across restarts.
    ...
    async fn api_key_login(&self) -> Result<(), LlmError> {
        ...
        // Persist to ~/.threatclaw/.env so the key survives restarts
