...
impl CodexChatGptProvider {
    ...
    /// Convert ThreatClaw messages to Responses API request JSON.
    ...
    /// `"timestamp": ""`). ThreatClaw's tool validation treats these as
    ...
    async fn complete_with_tools(
        ...
        let tool_calls: Vec<ToolCall> = result
            ...
            .map(|tc| {
                ...
                // `"timestamp": ""`), which ThreatClaw's tool validation rejects.
