...
pub async fn lookup_domain(domain: &str, api_key: &str) -> Result<SecurityTrailsResult, String> {
    ...
    let domain_resp = client.get(&domain_url)
    ...
    let domain_body: serde_json::Value = domain_resp.json().await
        .map_err(|e| format!("SecurityTrails parse: {}", e))?;

    let dns = &domain_body["current_dns"];

    let a_records: Vec<String> = dns["a"]["values"].as_array()
        .map(|a| a.iter().filter_map(|v| v["ip"].as_str().map(String::from)).collect())
        .unwrap_or_default();

    let mx_records: Vec<String> = dns["mx"]["values"].as_array()
        .map(|a| a.iter().filter_map(|v| v["hostname"].as_str().map(String::from)).collect())
        .unwrap_or_default();

    let ns_records: Vec<String> = dns["ns"]["values"].as_array()
        .map(|a| a.iter().filter_map(|v| v["nameserver"].as_str().map(String::from)).collect())
        .unwrap_or_default();

    // Get subdomains (separate request)
    let sub_url = format!("{}/domain/{}/subdomains", API_URL, domain);
    let sub_resp = client.get(&sub_url)
        .header("APIKEY", api_key)
        .timeout(std::time::Duration::from_secs(15))
        .send().await
    ...
    let subdomains: Vec<String> = if sub_resp.status().is_success() {
        let sub_body: serde_json::Value = sub_resp.json().await
            .map_err(|e| format!("SecurityTrails subdomains parse: {}", e))?;
        sub_body["subdomains"].as_array()
            .map(|a| {
                a.iter()
...
pub async fn dns_history(domain: &str, api_key: &str, record_type: &str) -> Result<Vec<DnsHistoryEntry>, String> {
    if api_key.is_empty() {
        return Err("SecurityTrails API key required".into());
    }
    ...
    let entries = body["records"].as_array()
        .map(|arr| {
            arr.iter().map(|r| {
                ...
                DnsHistoryEntry {
                    ...
                    values,
                    organizations: r["organizations"].as_array()
