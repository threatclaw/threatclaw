...
const CRITICAL_SOFTWARE: &[&str] = &[
    "openssh", "openssl", "nginx", "apache", "httpd",
    "php", "python", "node", "java", "tomcat",
    "postgresql", "mysql", "mariadb", "redis", "mongodb",
    "docker", "containerd", "kubernetes", "kubelet",
    "samba", "bind", "named", "postfix", "exim",
    "sudo", "polkit", "systemd", "kernel", "linux-image",
    "curl", "wget", "git", "vscode", "chrome", "firefox",
    "wordpress", "drupal", "joomla",
    "exchange", "iis", "rdp", "smb",
];
...
pub async fn scan_asset_software(
    ...
    let mut result = VulnScanResult {
        software_checked: 0, cves_found: 0, findings_created: 0, critical_count: 0,
    };

    if software.is_empty() { return result; }
    ...
    for sw in software {
        ...
        for cve_id in &kev_cves {
            if existing_cves.contains(cve_id) { continue; }

            let _ = store.insert_finding(&crate::db::threatclaw_store::NewFinding {
                skill_id: "software-vuln".into(),
                title: format!("{} {} — {} (CISA KEV: exploit actif)", name, version, cve_id),
        ...
        if CRITICAL_SOFTWARE.iter().any(|cs| name_lower.contains(cs)) {
            ...
            for (cve_id, cvss, severity) in &cached_cves {
                ...
                let sev = if *cvss >= 9.0 { "CRITICAL" } else if *cvss >= 7.0 { "HIGH" } else { "MEDIUM" };
                let _ = store.insert_finding(&crate::db::threatclaw_store::NewFinding {
                    skill_id: "software-vuln".into(),
    ...
    if result.findings_created > 0 {
        tracing::info!(
            ...
            asset_name, asset_id, result.software_checked, software.len(),
            result.cves_found, result.findings_created, result.critical_count
        );
    }
...
async fn check_kev_for_software(store: &dyn Database, name: &str, version: &str) -> Vec<String> {
    ...
    if let Ok(entries) = store.list_settings("_kev").await {
        for row in &entries {
            if !row.key.starts_with("CVE-") { continue; }
...
async fn check_nvd_cache_for_software(
    ...
    if let Ok(Some(cached)) = store.get_enrichment_cache("nvd_software", &name.to_lowercase()).await {
...
pub async fn scan_all_assets(store: std::sync::Arc<dyn Database>) -> usize {
    ...
    for asset in &assets {
        ...
        if let Some(sw_array) = software.as_array() {
            if sw_array.is_empty() { continue; }
            let result = scan_asset_software(store.as_ref(), &asset.id, &asset.name, sw_array).await;
            total_findings += result.findings_created;
        }
    }

    if total_findings > 0 {
        tracing::info!("SOFTWARE-VULN: Daily scan complete — {} findings across {} assets", total_findings, assets.len());
    }
