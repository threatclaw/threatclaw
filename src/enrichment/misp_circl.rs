...
pub async fn sync_feed(store: &dyn crate::db::Database) -> Result<usize, String> {
    ...
    // 1. Fetch manifest
    let manifest: serde_json::Value = c.get(MANIFEST_URL)
        .send().await.map_err(|e| format!("MISP manifest: {e}"))?
        .json().await.map_err(|e| format!("MISP manifest JSON: {e}"))?;
    ...
    tracing::info!("MISP: {} recent events (last {}d) to fetch", recent_uuids.len(), DAYS_BACK);
    ...
    let ioc_types: HashSet<&str> = ["ip-dst", "ip-src", "domain", "hostname", "url",
        "sha256", "sha1", "md5", "filename|sha256", "filename|md5"].into_iter().collect();

    for uuid in &recent_uuids {
        if tokio::time::Instant::now() > deadline {
            tracing::warn!("MISP: Deadline reached, stopping after {} events", events_processed);
            break;
        }
        ...
        let resp = match c.get(&url).send().await {
            ...
            Ok(r) => { tracing::debug!("MISP: Event {} returned {}", uuid, r.status()); continue; }
            Err(e) => { tracing::debug!("MISP: Event {} fetch failed: {}", uuid, e); continue; }
        };
        ...
        for attr in attributes {
            ...
            if !ioc_types.contains(attr_type) { continue; }
            ...
            match attr_type {
                "ip-dst" | "ip-src" => { all_ips.insert(clean_value); }
                "domain" | "hostname" => { all_domains.insert(clean_value); }
                "url" => { all_urls.insert(clean_value); }
                "sha256" | "sha1" | "md5" | "filename|sha256" | "filename|md5" => { all_hashes.insert(clean_value); }
                _ => {}
            }
        }
    ...
    store.set_setting("_enrichment", "misp_iocs", &data).await
        .map_err(|e| format!("MISP store: {e}"))?;

    tracing::info!("MISP: Synced {} IoC from {} events", total, events_processed);
    Ok(total)
}

/// Load MISP IoC into a Bloom filter.
pub async fn load_into_bloom(store: &dyn crate::db::Database, filter: &mut crate::agent::ioc_bloom::BloomFilter) {
    if let Ok(Some(data)) = store.get_setting("_enrichment", "misp_iocs").await {
        ...
        for key in &["ips", "domains", "urls", "hashes"] {
            if let Some(arr) = data[key].as_array() {
                for v in arr {
                    if let Some(s) = v.as_str() { filter.insert(s); count += 1; }
                }
            }
        }
        if count > 0 {
