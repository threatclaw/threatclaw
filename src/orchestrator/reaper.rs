...
//!
//! The SandboxReaper periodically scans Docker for ThreatClaw-labeled containers
...
//! 1. Scans Docker for containers with the `threatclaw.job_id` label
...
impl Default for ReaperConfig {
    fn default() -> Self {
        Self {
            ...
            container_label: "threatclaw.job_id".to_string(),
        }
    }
}
...
impl SandboxReaper {
    ...
    async fn scan_and_reap(&self) {
        let containers = match self.list_threatclaw_containers().await {
            Ok(c) => c,
            Err(e) => {
    ...
    /// List all ThreatClaw-managed containers from Docker.
    ///
    /// Returns tuples of (container_id, job_id, created_at).
    async fn list_threatclaw_containers(
        &self,
                ...
                .and_then(|s| s.parse::<Uuid>().ok())
            {
                ...
                None => {
                    tracing::warn!(
                        ...
                        "Reaper: threatclaw container missing valid job_id label"
                    );
                    continue;
                }
            };
...
mod tests {
    ...
    fn parse_container_labels_extracts_job_id_and_timestamp() {
        ...
        labels.insert("threatclaw.job_id".to_string(), job_id.to_string());
        labels.insert(
            "threatclaw.created_at".to_string(),
            "2024-01-15T10:30:45+00:00".to_string(),
        );

        // Verify parsing works
        let parsed_id: Option<Uuid> = labels
            .get("threatclaw.job_id")
            .and_then(|s| s.parse::<Uuid>().ok());
    ...
    fn reaper_config_defaults_are_reasonable() {
        ...
        assert_eq!(cfg.container_label, "threatclaw.job_id");
    }
    ...
    // They require Docker to be running and the THREATCLAW_E2E_DOCKER_TESTS
    // environment variable to be set (to avoid running them in CI by default).
    //
    // Run with: THREATCLAW_E2E_DOCKER_TESTS=1 cargo test orchestrator::reaper::e2e_tests --lib -- --nocapture
    ...
    mod e2e_tests {
        ...
        fn should_run_e2e() -> bool {
            std::env::var("THREATCLAW_E2E_DOCKER_TESTS").is_ok()
        }

        /// Test that reaper can list containers with ThreatClaw labels
        #[tokio::test]
        async fn e2e_reaper_lists_threatclaw_containers() {
            if !should_run_e2e() {
                eprintln!("Skipping e2e test (set THREATCLAW_E2E_DOCKER_TESTS=1 to run)");
                return;
            }
            ...
            // Create a test container with ThreatClaw labels
            let job_id = Uuid::new_v4();
            let test_name = format!("threatclaw-reaper-test-{}", &job_id.to_string()[..8]);
            ...
            labels_str.insert("threatclaw.job_id", &job_id_str);
            labels_str.insert("threatclaw.created_at", &created_at_str);
            ...
            assert!(
                labels.contains_key("threatclaw.job_id"),
                "Container should have threatclaw.job_id label"
            );
            assert_eq!(
                labels.get("threatclaw.job_id").map(|s| s.as_str()),
        ...
        async fn e2e_reaper_removes_orphaned_containers() {
            ...
            let mut labels: std::collections::HashMap<&str, &str> =
                std::collections::HashMap::new();
            labels.insert("threatclaw.job_id", &job_id_str);
            labels.insert("threatclaw.created_at", &created_at_str);
        ...
        async fn e2e_reaper_respects_age_threshold() {
            ...
            let mut old_labels: std::collections::HashMap<&str, &str> =
                std::collections::HashMap::new();
            old_labels.insert("threatclaw.job_id", &old_id_str);
            old_labels.insert("threatclaw.created_at", &old_time_str);
            ...
            let mut new_labels: std::collections::HashMap<&str, &str> =
                std::collections::HashMap::new();
            new_labels.insert("threatclaw.job_id", &new_id_str);
            new_labels.insert("threatclaw.created_at", &new_time_str);

            let mut containers_to_cleanup = Vec::new();

            // Create old container
            let old_name = format!("threatclaw-age-old-{}", &old_job_id.to_string()[..8]);
            if let Ok(r) = docker
                .create_container(
            ...
            let new_name = format!("threatclaw-age-new-{}", &new_job_id.to_string()[..8]);
            if let Ok(r) = docker
                .create_container(
                    Some(bollard::container::CreateContainerOptions {
