...
pub async fn predict_attack_paths(store: &dyn Database) -> AttackPathAnalysis {
    ...
    let summary = if total == 0 {
        ...
        format!(
            "{} chemins d'attaque prédits ({} critiques). Top menace : {}",
            total,
            critical,
            paths.first().map(|p| p.entry_point.as_str()).unwrap_or("?")
        )
    };

    if total > 0 {
        tracing::info!(
            "ATTACK_PATH: {} paths predicted ({} critical)",
            total,
            critical
        );
    }
        ...
        .filter_map(|r| {
            ...
            Some(AttackPath {
                ...
                path: vec![
                    PathNode {
                        asset: pivot_id.into(),
                        hostname: pivot_host.into(),
                        role: "pivot".into(),
                        cvss_max: 0.0,
                        has_kev: false,
                    },
                    PathNode {
                        asset: target_id.into(),
                        hostname: target_host.into(),
                        role: "target".into(),
                        cvss_max: 0.0,
                        has_kev: false,
                    },
    ...
    results
        ...
        .filter_map(|r| {
            ...
            Some(AttackPath {
                ...
                cves_involved: vec![entry_cve.into(), target_cve.into()]
                    .into_iter()
                    .filter(|s: &String| !s.is_empty())
                    .collect(),
                mitre_techniques: vec!["T1190".into()], // Exploit Public-Facing
                risk: if has_kev || cvss >= 9.0 {
                    "critical".into()
                } else {
                    "high".into()
                },
...
fn generate_recommendations(paths: &[AttackPath]) -> Vec<String> {
    ...
    if !kev_nodes.is_empty() {
        recs.push(format!(
            "URGENT : Patcher les CVEs CISA KEV sur {}",
            kev_nodes
                .iter()
    ...
    let direct: Vec<_> = paths
        .iter()
        .filter(|p| p.path.len() == 1 && p.risk == "critical")
        .collect();
    if !direct.is_empty() {
        recs.push(format!(
            "Isoler les assets critiques attaqués directement : {}",
            direct
                .iter()
