...
pub async fn compute_blast_radius(store: &dyn Database, asset_id: &str) -> BlastRadius {
    ...
    hops.push(HopImpact { hop: 1, count: hop1.len(), assets: hop1.clone() });
    ...
    let hop2_new: Vec<_> = hop2.into_iter()
        .filter(|a| !all_assets.iter().any(|x| x.id == a.id) && a.id != asset_id)
        .collect();
    hops.push(HopImpact { hop: 2, count: hop2_new.len(), assets: hop2_new.clone() });
    ...
    let hop3_new: Vec<_> = hop3.into_iter()
        .filter(|a| !all_assets.iter().any(|x| x.id == a.id) && a.id != asset_id)
        .collect();
    hops.push(HopImpact { hop: 3, count: hop3_new.len(), assets: hop3_new.clone() });
    all_assets.extend(hop3_new);

    let total = all_assets.len();
    let critical = all_assets.iter().filter(|a| a.criticality == "critical").count();
    ...
    if total > 0 {
        tracing::info!("BLAST_RADIUS: {} → {} assets impactés ({} critiques), score {:.1}",
            asset_id, total, critical, impact_score);
    }
...
async fn find_shared_vulnerability_assets(store: &dyn Database, asset_id: &str) -> Vec<AssetImpact> {
    let results = query(store, &format!(
    ...
    results.iter().filter_map(|r| {
        ...
        Some(AssetImpact { id, hostname, criticality, cve_count: 1, has_kev })
    }).collect()
}
...
fn parse_asset_results(results: &[serde_json::Value]) -> Vec<AssetImpact> {
    results.iter().filter_map(|r| {
        ...
        let hostname = result["other.hostname"].as_str().unwrap_or(&id).to_string();
        let criticality = result["other.criticality"].as_str().unwrap_or("medium").to_string();
        Some(AssetImpact { id, hostname, criticality, cve_count: 0, has_kev: false })
    }).collect()
}
...
pub fn format_blast_radius(br: &BlastRadius) -> String {
    if br.total_impacted == 0 { return String::new(); }

    let mut out = format!("BLAST RADIUS — {}\n", br.summary);
    for hop in &br.hops {
        if hop.count > 0 {
            let names: Vec<_> = hop.assets.iter().map(|a| a.hostname.as_str()).collect();
            out.push_str(&format!("  Hop {} : {} assets ({})\n", hop.hop, hop.count, names.join(", ")));
        }
    }
...
mod tests {
    ...
    fn test_impact_score() {
        let assets = vec![
            AssetImpact { id: "a".into(), hostname: "srv-db".into(), criticality: "critical".into(), cve_count: 0, has_kev: true },
            AssetImpact { id: "b".into(), hostname: "srv-web".into(), criticality: "medium".into(), cve_count: 0, has_kev: false },
        ];
    ...
    fn test_format_empty() {
        let br = BlastRadius {
            source_asset: "test".into(), total_impacted: 0, critical_impacted: 0,
            hops: vec![], impact_score: 0.0, summary: String::new(), recommendation: String::new(),
        };
