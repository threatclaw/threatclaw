...
const KNOWN_APTS: &[KnownApt] = &[
    KnownApt { name: "APT28 (Fancy Bear)", country: "RU",
        techniques: &["T1110", "T1078", "T1566", "T1071", "T1021"] },
    KnownApt { name: "APT29 (Cozy Bear)", country: "RU",
        techniques: &["T1190", "T1059", "T1071", "T1003", "T1041"] },
    KnownApt { name: "Lazarus Group", country: "KP",
        techniques: &["T1566", "T1204", "T1059", "T1071", "T1041"] },
    KnownApt { name: "APT41 (Winnti)", country: "CN",
        techniques: &["T1190", "T1059", "T1068", "T1003", "T1021"] },
    KnownApt { name: "Sandworm", country: "RU",
        techniques: &["T1190", "T1059", "T1068", "T1071.004", "T1041"] },
    KnownApt { name: "Turla", country: "RU",
        techniques: &["T1071.004", "T1041", "T1059", "T1003", "T1078"] },
    KnownApt { name: "MuddyWater", country: "IR",
...
pub async fn profile_threat_actors(store: &dyn Database) -> ThreatActorAnalysis {
    ...
    for (country, ips) in &country_groups {
        ...
        let actor_name = if let Some(ref apt) = apt_match {
            if apt.similarity_score >= 70 {
                format!("Probable {} ({}% match)", apt.apt_name, apt.similarity_score)
            } else {
    ...
    let attributed = actors.iter()
        .filter(|a| a.apt_similarity.as_ref().map(|m| m.similarity_score >= 60).unwrap_or(false))
        .count();
    ...
    let summary = if total == 0 {
        ...
        format!("{} acteurs profilés, {} avec attribution APT probable", total, attributed)
    };

    if total > 0 {
        tracing::info!("THREAT_ACTOR: {} actors profiled ({} attributed)", total, attributed);
    }
...
async fn get_techniques_for_ips(store: &dyn Database, ips: &[String]) -> Vec<String> {
    ...
    let results = query(store,
        "MATCH (t:Technique) RETURN t.mitre_id LIMIT 50"
    ).await;
...
fn match_known_apt(country: &str, techniques: &[String]) -> Option<AptMatch> {
    ...
    for apt in KNOWN_APTS {
        ...
        let matching_techniques: Vec<String> = techniques.iter()
            .filter(|t| apt.techniques.contains(&t.as_str()))
            .cloned()
            .collect();

        if matching_techniques.is_empty() && !country_match { continue; }

        let technique_ratio = if apt.techniques.is_empty() { 0.0 }
            else { matching_techniques.len() as f64 / apt.techniques.len() as f64 };

        let mut score = (technique_ratio * 70.0) as u8;
        if country_match { score = score.saturating_add(25); }
        if matching_techniques.len() >= 3 { score = score.saturating_add(5); }
        score = score.min(100);
