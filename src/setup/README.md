...
This document is the authoritative specification for ThreatClaw's onboarding
...
3. If Command::Run or no command:
   a. Load .env files (dotenvy::dotenv() then load_threatclaw_env())
...
`DATABASE_BACKEND` live in `~/.threatclaw/.env`.
...
```
auto_setup_database()    → libsql at ~/.threatclaw/threatclaw.db (zero prompts)
...
Step 2/2: Model Selection     ← only interactive step
       ↓
   save_and_summarize()      → includes tip to run `threatclaw onboard`
```
...
defaults to libsql at `~/.threatclaw/threatclaw.db`, creates the database,
...
The full 9-step wizard remains available via `threatclaw onboard`.
...
**libSQL path:**
1. Offer local path (default: `~/.threatclaw/threatclaw.db`)
...
2. "Allow threatclaw to access this keychain item" (per-app authorization)
...
- **Never probe the keychain in read-only commands** (e.g., `threatclaw status`).
...
- **NEAR AI Chat** path: session token saved to `~/.threatclaw/session.json`.
  Hosting providers can set `NEARAI_SESSION_TOKEN` env var directly (takes
  precedence over file-based tokens).
- **NEAR AI Cloud** path: `NEARAI_API_KEY` saved to `~/.threatclaw/.env`
...
6b. Discover WASM channels from ~/.threatclaw/channels/
...
1. Already installed in `~/.threatclaw/channels/`
...
4. Discover already-installed tools in `~/.threatclaw/tools/`
...
**Layer 1: `~/.threatclaw/.env`** (bootstrap vars)
...
LIBSQL_PATH="/Users/name/.threatclaw/threatclaw.db"
...
DATABASE_URL="postgres://user:pass@localhost/threatclaw"
LLM_BACKEND="nearai"
```
...
1. Writes bootstrap vars to `~/.threatclaw/.env` via `write_bootstrap_env()`
...
Bootstrap vars written to `~/.threatclaw/.env`:
...
pub struct Settings {
    ...
    // Advanced (not in wizard, set via `threatclaw config set`)
...
- Service name: `"threatclaw"`, account: `"master_key"`
...
1. **NEAR AI Cloud API key (option 4 in auth menu):** Get an API key
   ...
   local listener is needed. The key is saved to `~/.threatclaw/.env`
   and the encrypted secrets store. Uses the OpenAI-compatible
   ChatCompletions API mode.

2. **Custom callback URL:** Set `THREATCLAW_OAUTH_CALLBACK_URL` to a
   publicly accessible URL (e.g., via SSH tunnel or reverse proxy) that
   forwards to port 9876 on the server:
   ```bash
   export THREATCLAW_OAUTH_CALLBACK_URL=https://myserver.example.com:9876
   ```
...
6. Run the full shipping checklist:
   ...
   ```
7. Test a fresh onboarding: `rm -rf ~/.threatclaw && cargo run`
