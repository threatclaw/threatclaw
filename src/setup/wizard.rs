...
/// Interactive setup wizard for ThreatClaw.
...
impl SetupWizard {
    ...
    pub async fn run(&mut self) -> Result<(), SetupError> {
        print_banner();
        print_header("ThreatClaw Setup Wizard");
    ...
    async fn reconnect_existing_db(&mut self) -> Result<(), SetupError> {
        ...
        Err(SetupError::Database(
            "No database configured. Run full setup first (threatclaw onboard).".to_string(),
        ))
    }
    ...
    async fn reconnect_postgres(&mut self) -> Result<(), SetupError> {
        let url = std::env::var("DATABASE_URL").map_err(|_| {
            SetupError::Database(
                "DATABASE_URL not set. Run full setup first (threatclaw onboard).".to_string(),
            )
        })?;
    ...
    async fn step_database_libsql(&mut self) -> Result<(), SetupError> {
        ...
        println!();
        print_info("ThreatClaw uses an embedded SQLite database (libSQL).");
    ...
    async fn test_database_connection_postgres(&mut self, url: &str) -> Result<(), SetupError> {
        ...
        if major_version < MIN_PG_MAJOR_VERSION {
            return Err(SetupError::Database(format!(
                "PostgreSQL {} detected. ThreatClaw requires PostgreSQL {} or later for pgvector support.\n\
        ...
        if pgvector_row.is_none() {
            return Err(SetupError::Database(format!(
                "pgvector extension not found on your PostgreSQL server.\n\n\
                 ...
                 Then restart PostgreSQL and re-run: threatclaw onboard",
                major_version
            )));
        }
    ...
    async fn step_security(&mut self) -> Result<(), SetupError> {
        ...
        match choice {
            ...
            1 => {
                ...
                // Store hex for write_bootstrap_env to persist to ~/.threatclaw/.env.
                self.settings.secrets_master_key_hex = Some(key_hex.clone());

                println!();
                print_info("Master key generated and will be saved to ~/.threatclaw/.env");
                println!();
    ...
    async fn auto_setup_security(&mut self) -> Result<(), SetupError> {
        ...
        self.settings.secrets_master_key_source = KeySource::Env;
        print_success("Master key stored in ~/.threatclaw/.env");
        Ok(())
    }
    ...
    async fn step_channels(&mut self) -> Result<(), SetupError> {
        ...
        let channels_dir = threatclaw_base_dir().join("channels");
    ...
    async fn step_extensions(&mut self) -> Result<(), SetupError> {
        let catalog = match load_registry_catalog() {
            ...
            None => {
                ...
                print_info("Install tools manually with: threatclaw tool install <path>");
                return Ok(());
            }
        };
        ...
        print_info("  threatclaw registry install <name>");
        println!();

        // Check which tools are already installed
        let tools_dir = threatclaw_base_dir().join("tools");
            ...
            match installer.install_with_source_fallback(tool, false).await {
                Ok(outcome) => {
                        ...
                        && auth.method.is_some()
                    {
                        ...
                        let hint = format!("  {} - threatclaw tool auth {}", provider, tool.name);
                        if !auth_needed
                            .iter()
    ...
    async fn step_docker_sandbox(&mut self) -> Result<(), SetupError> {
        print_info("ThreatClaw can execute code, run builds, and use tools inside Docker");
    ...
    /// Write bootstrap environment variables to `~/.threatclaw/.env`.
    ///
    ...
    /// Save settings to the database and `~/.threatclaw/.env`, then print summary.
    async fn save_and_summarize(&mut self) -> Result<(), SetupError> {
        ...
        println!("  threatclaw config set <setting> <value>");
        println!("  threatclaw onboard");
        println!();

        if self.config.quick {
            print_info(
                "Tip: Run `threatclaw onboard` to configure channels, extensions, embeddings, and more.",
            );
            println!();
        }
...
async fn install_selected_registry_channels(
    ...
    for name in selected_channels {
        ...
        let installer = crate::registry::installer::RegistryInstaller::new(
            repo_root.clone(),
            threatclaw_base_dir().join("tools"),
...
mod tests {
    ...
    fn test_try_with_config_and_toml_propagates_invalid_owner_env() {
        ...
        unsafe {
            if let Some(value) = original {
                std::env::set_var("THREATCLAW_OWNER_ID", value);
            } else {
                std::env::remove_var("THREATCLAW_OWNER_ID");
            }
        }
    ...
    async fn test_discover_wasm_channels_nonexistent_dir() {
        let channels = discover_wasm_channels(
            &std::env::temp_dir().join("threatclaw_nonexistent_dir_abcxyz123"),
        )
        .await;
        assert!(channels.is_empty());
    }
