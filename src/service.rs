//! OS service management for running ThreatClaw as a daemon.
//!
//! Generates and manages platform-native service definitions:
//! - **macOS**: launchd plist at `~/Library/LaunchAgents/com.threatclaw.daemon.plist`
//! - **Linux**: systemd user unit at `~/.config/systemd/user/threatclaw.service`
//!
//! The installed service runs `threatclaw run` (the default agent mode) and is
...
const SERVICE_LABEL: &str = "com.threatclaw.daemon";
const SYSTEMD_UNIT: &str = "threatclaw.service";
...
fn install_macos() -> Result<()> {
    ...
    let logs_dir = threatclaw_logs_dir();
    ...
    println!("  Start with: threatclaw service start");
    Ok(())
}
...
fn start() -> Result<()> {
    if cfg!(target_os = "macos") {
        ...
        if !plist.exists() {
            bail!("Service not installed. Run `threatclaw service install` first.");
        }
...
fn threatclaw_logs_dir() -> PathBuf {
    threatclaw_base_dir().join("logs")
}
...
mod tests {
    ...
    fn macos_plist_path_has_expected_suffix() {
        ...
        assert!(
            s.ends_with("Library/LaunchAgents/com.threatclaw.daemon.plist"),
            "unexpected path: {s}"
        );
    }
    ...
    fn linux_unit_path_has_expected_suffix() {
        ...
        let s = path.to_string_lossy();
        assert!(
            s.ends_with(".config/systemd/user/threatclaw.service"),
            "unexpected path: {s}"
        );
    }

    #[test]
    fn logs_dir_under_threatclaw() {
        let path = threatclaw_logs_dir();
        let s = path.to_string_lossy();
        assert!(s.ends_with(".threatclaw/logs"), "unexpected path: {s}");
    }

    #[test]
    fn macos_plist_sets_cli_enabled_false() {
        let plist = macos_plist_content("/tmp/threatclaw", "/tmp/stdout.log", "/tmp/stderr.log");
