...
fn should_attempt_source_fallback(err: &RegistryError) -> bool {
    match err {
        ...
        RegistryError::ChecksumMismatch { url, .. } => {
            url.contains("github.com/nearai/threatclaw/releases/latest/")
        }
...
pub struct RegistryInstaller {
    ...
    repo_root: PathBuf,
    /// Directory for installed tools (`~/.threatclaw/tools/`).
    tools_dir: PathBuf,
    /// Directory for installed channels (`~/.threatclaw/channels/`).
...
impl RegistryInstaller {
    ...
    pub fn with_defaults(repo_root: PathBuf) -> Self {
        let base_dir = threatclaw_base_dir();
        Self {
            repo_root,
    ...
    pub async fn install_from_source(
        ...
        // CLI commands (`threatclaw tool auth <name>`) all agree on the stem.
    ...
    pub async fn install_bundle(
        ...
        if let Some(shared) = &bundle.shared_auth {
            auth_hints.push(format!(
                "Bundle uses shared auth '{}'. Run `threatclaw tool auth <any-member>` to authenticate all members.",
                shared
            ));
        }
...
mod tests {
    ...
    fn test_installer_creation() {
        let installer = RegistryInstaller::new(
            PathBuf::from("/repo"),
            PathBuf::from("/home/.threatclaw/tools"),
            PathBuf::from("/home/.threatclaw/channels"),
        );
    ...
    async fn test_install_from_artifact_rejects_null_sha256() {
        ...
        let manifest = test_manifest(
            ...
            "tools-src/demo",
            Some(
                "https://github.com/nearai/threatclaw/releases/latest/download/demo-wasm32-wasip2.tar.gz".to_string(),
            ),
            None, // sha256 = null
        );
    ...
    fn test_should_attempt_source_fallback_policy() {
        let download = RegistryError::DownloadFailed {
            url: "https://github.com/nearai/threatclaw/releases/latest/download/demo.wasm"
                .to_string(),
    ...
    fn test_source_fallback_on_latest_url_mismatch() {
        let latest_mismatch = RegistryError::ChecksumMismatch {
            url: "https://github.com/nearai/threatclaw/releases/latest/download/github-wasm32-wasip2.tar.gz".to_string(),
        ...
        let pinned_mismatch = RegistryError::ChecksumMismatch {
            url: "https://github.com/nearai/threatclaw/releases/download/v0.7.0/github-0.2.0-wasm32-wasip2.tar.gz".to_string(),
            expected_sha256: "aaa".to_string(),
            actual_sha256: "bbb".to_string(),
        };
        assert!(
