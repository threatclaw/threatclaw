...
impl LlmProvider for MockLlmProvider {
    ...
    async fn complete(&self, req: CompletionRequest) -> Result<CompletionResponse, LlmError> {
        ...
        let user_msg = req
            ...
            .iter()
            .rev()
            .find(|m| m.role == threatclaw::llm::Role::User)
    ...
    async fn complete_with_tools(
        ...
        if let Some(tool) = req.tools.first() {
            Ok(ToolCompletionResponse {
                content: None,
                tool_calls: vec![threatclaw::llm::ToolCall {
...
async fn start_test_server_with_provider(
    ...
    let state = Arc::new(GatewayState {
        ...
        skill_registry: None,
        skill_catalog: None,
        chat_rate_limiter: threatclaw::channels::web::server::RateLimiter::new(30, 60),
        oauth_rate_limiter: threatclaw::channels::web::server::RateLimiter::new(10, 60),
...
async fn test_chat_completions_streaming() {
    ...
    assert_eq!(
        resp.headers()
            .get("x-threatclaw-streaming")
            .and_then(|v| v.to_str().ok()),
        Some("simulated"),
        "Expected x-threatclaw-streaming: simulated header"
    );
...
async fn test_chat_completions_body_too_large() {
    ...
    let state = threatclaw::channels::web::test_helpers::TestGatewayBuilder::new()
        .llm_provider(llm_provider)
        .build();
    let auth_state = threatclaw::channels::web::auth::AuthState {
    ...
    let app = Router::new()
        .route(
            "/v1/chat/completions",
            post(threatclaw::channels::web::openai_compat::chat_completions_handler),
        )
        .route_layer(middleware::from_fn_with_state(
            auth_state,
            threatclaw::channels::web::auth::auth_middleware,
        ))
