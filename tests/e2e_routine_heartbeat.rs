...
mod tests {
    ...
    use threatclaw::agent::{HeartbeatConfig, HeartbeatRunner};
    use threatclaw::channels::IncomingMessage;
    use threatclaw::config::{RoutineConfig, SafetyConfig};
    use threatclaw::db::Database;
    use threatclaw::safety::SafetyLayer;
    use threatclaw::tools::ToolRegistry;
    use threatclaw::workspace::Workspace;
    use threatclaw::workspace::hygiene::HygieneConfig;
    ...
    async fn system_event_trigger_matches_and_filters() {
        ...
        filters.insert("repository".to_string(), "nearai/threatclaw".to_string());
        ...
        let fired_wrong_type = engine
            .emit_system_event(
                "github",
                "issue.closed",
                &serde_json::json!({"repository": "nearai/threatclaw"}),
                Some("default"),
            )
            .await;
        assert_eq!(
    ...
    async fn heartbeat_findings() {
        ...
        match result {
            threatclaw::agent::HeartbeatResult::NeedsAttention(msg) => {
                assert!(
                    msg.contains("error"),
    ...
    async fn heartbeat_empty_skip() {
        ...
        let result = runner.check_heartbeat().await;
        assert!(
            matches!(result, threatclaw::agent::HeartbeatResult::Skipped),
