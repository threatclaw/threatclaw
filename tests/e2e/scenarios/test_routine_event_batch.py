...
@pytest.mark.asyncio
async def test_create_event_trigger_routine(page, threatclaw_server):
...
async def test_event_trigger_fires_on_matching_message(
    ...
    completed_run = await _wait_for_completed_run(threatclaw_server, routine["id"])
...
async def test_event_trigger_skips_non_matching_message(
    ...
    assert await _get_routine_runs(threatclaw_server, routine["id"]) == []
...
async def test_channel_filter_applied_correctly(
    ...
    http_run = await _wait_for_completed_run(threatclaw_server, http_routine["id"])
    await asyncio.sleep(2)
    telegram_runs = await _get_routine_runs(threatclaw_server, telegram_routine["id"])
