...
The conftest sets THREATCLAW_OAUTH_CALLBACK_URL (non-loopback, forces gateway
mode) and THREATCLAW_OAUTH_EXCHANGE_URL (points to mock_llm.py's /oauth/exchange).
"""
...
async def test_oauth_install_gmail(threatclaw_server):
...
async def test_oauth_configure_returns_auth_url(threatclaw_server):
...
async def test_oauth_activate_returns_auth_url(threatclaw_server):
    """Activate on un-authenticated gmail returns auth_url."""
    if not _gmail_installed:
        pytest.skip("gmail not installed")

    r = await api_post(
        threatclaw_server, "/api/extensions/gmail/activate", timeout=30
    )
    assert r.status_code == 200
    data = r.json()
...
async def test_oauth_callback_exchanges_token(threatclaw_server):
    ...
    async with httpx.AsyncClient() as client:
        r = await client.get(
            f"{threatclaw_server}/oauth/callback",
...
async def test_oauth_callback_replay_rejected(threatclaw_server):
...
async def test_oauth_callback_invalid_state(threatclaw_server):
...
async def test_oauth_extension_authenticated(threatclaw_server):
...
async def test_oauth_tools_registered(threatclaw_server):
...
async def test_remove_during_pending_oauth_invalidates_callback(threatclaw_server):
    ...
    remove_r = await api_post(
        threatclaw_server, "/api/extensions/gmail/remove", timeout=30
    )
...
async def test_cleanup_gmail(threatclaw_server):
