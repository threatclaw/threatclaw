...
async def test_telegram_setup_modal_shows_bot_token_field(page):
    ...
    assert "ThreatClaw will show a one-time code" in (
                ...
                body=json.dumps(
                    {
                        "success": True,
                        "activated": False,
                        "message": "Configuration saved for 'telegram'. Send `/start iclaw-7qk2m9` to @test_hot_bot in Telegram. ThreatClaw will finish setup automatically.",
                        "verification": {
                            "code": "iclaw-7qk2m9",
                            "instructions": "Send `/start iclaw-7qk2m9` to @test_hot_bot in Telegram. ThreatClaw will finish setup automatically.",
