...
async def test_pairing_list_returns_empty_for_unknown_channel(threatclaw_server):
    """GET /api/pairing/{channel} returns empty list or 404 for non-existent channel."""
    async with httpx.AsyncClient() as client:
        r = await client.get(
            f"{threatclaw_server}/api/pairing/nonexistent-channel",
            headers=_headers(),
            timeout=10,
        )
...
async def test_approve_invalid_code_rejected(threatclaw_server):
    """POST /api/pairing/{channel}/approve with bad code returns error."""
    async with httpx.AsyncClient() as client:
        r = await client.post(
            f"{threatclaw_server}/api/pairing/test-channel/approve",
            json={"code": "INVALID0"},
            headers=_headers(),
            timeout=10,
        )
...
async def test_approve_empty_code_rejected(threatclaw_server):
...
async def test_pairing_approve_requires_auth(threatclaw_server):
