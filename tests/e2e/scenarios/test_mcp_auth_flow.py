...
async def test_mcp_install(threatclaw_server, mock_llm_server):
    """Install a mock MCP server pointing at mock_llm.py's /mcp endpoint."""
    await _ensure_removed(threatclaw_server, "mock-mcp")
    ...
    ext = await _get_extension(threatclaw_server, "mock-mcp")
...
async def test_mcp_activate_triggers_auth(threatclaw_server):
...
async def test_mcp_oauth_callback(threatclaw_server):
...
async def test_mcp_authenticated_after_oauth(threatclaw_server):
...
async def test_mcp_tools_registered(threatclaw_server):
...
async def test_mcp_auth_mode_cleared_llm_turn_fires(threatclaw_server, page):
...
async def test_mcp_400_activate_triggers_auth(threatclaw_server, mock_llm_server):
    ...
    """
    await _ensure_removed(threatclaw_server, "mock-mcp-400")
...
async def test_mcp_400_oauth_discovery_returns_auth_url(threatclaw_server):
    ...
    """
    ext = await _get_extension(threatclaw_server, "mock-mcp-400")
    if ext is None:
...
async def test_mcp_400_full_oauth_roundtrip(threatclaw_server):
...
async def test_mcp_400_cleanup(threatclaw_server):
...
async def test_mcp_cleanup(threatclaw_server):
