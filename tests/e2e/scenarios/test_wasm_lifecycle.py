...
async def extension_lifecycle_cleanup(threatclaw_server):
    """Start and end the module with a clean extension set."""
    await _ensure_removed(threatclaw_server, "web-search")
    await _ensure_removed(threatclaw_server, "gmail")
    yield
    await _ensure_removed(threatclaw_server, "web-search")
    await _ensure_removed(threatclaw_server, "gmail")
...
async def web_search_installed(threatclaw_server, extension_lifecycle_cleanup):
    """Install web-search once for tests that require the pre-configure state."""
    data = await _install_extension(threatclaw_server, "web-search")
    return {"name": "web-search", "install": data}


@pytest.fixture(scope="module")
async def web_search_configured(threatclaw_server, web_search_installed):
...
@pytest.fixture(scope="module")
async def gmail_installed(threatclaw_server, extension_lifecycle_cleanup):
    """Install gmail once for multi-extension and OAuth setup assertions."""
    data = await _install_extension(threatclaw_server, "gmail")
    return {"name": "gmail", "install": data}


@pytest.fixture(scope="module")
async def web_search_removed(threatclaw_server, web_search_configured):
    """Remove web-search once for post-uninstall assertions."""
    r = await api_post(
        threatclaw_server, "/api/extensions/web-search/remove", timeout=30
    )
    assert r.status_code == 200
    data = r.json()
...
@pytest.fixture(scope="module")
async def web_search_reinstalled(threatclaw_server, web_search_removed):
...
async def test_registry_lists_extensions(threatclaw_server):
    """Registry endpoint returns entries from the embedded catalog."""
    r = await api_get(threatclaw_server, "/api/extensions/registry")
    assert r.status_code == 200
    data = r.json()
...
async def test_registry_entry_fields(threatclaw_server):
...
async def test_registry_installed_flag_false_initially(threatclaw_server):
...
async def test_registry_search_filters(threatclaw_server):
    """Search query filters registry results."""
    r = await api_get(
        threatclaw_server, "/api/extensions/registry", params={"query": "search"}
    )
    assert r.status_code == 200
    entries = r.json()["entries"]
...
async def test_registry_search_no_match(threatclaw_server):
...
async def test_installed_extension_fields(threatclaw_server, web_search_installed):
    """After install, extension list shows correct fields."""
    ext = await _get_extension(threatclaw_server, "web-search")
...
async def test_installed_in_registry(threatclaw_server, web_search_installed):
...
async def test_setup_schema_has_secrets(threatclaw_server, web_search_installed):
    """Setup schema returns brave_api_key with correct field info."""
    r = await api_get(threatclaw_server, "/api/extensions/web-search/setup")
    assert r.status_code == 200
    data = r.json()
...
async def test_extension_not_authenticated_before_configure(
    threatclaw_server, web_search_installed
):
...
async def test_activate_before_configure_rejected(threatclaw_server, web_search_installed):
    """Activating a tool that needs setup secrets is rejected."""
    r = await api_post(
        threatclaw_server, "/api/extensions/web-search/activate", timeout=30
    )
    assert r.status_code == 200
    data = r.json()
...
async def test_configure_rejects_unknown_secret(threatclaw_server, web_search_installed):
...
async def test_extension_active_after_configure(threatclaw_server, web_search_configured):
...
async def test_setup_shows_provided(threatclaw_server, web_search_configured):
...
async def test_tools_registered_after_activate(
    threatclaw_server, web_search_configured
):
    """After activation, extension tools appear in the tools endpoint."""
    r = await api_get(threatclaw_server, "/api/extensions/tools")
    assert r.status_code == 200
...
async def test_configure_empty_secret_skipped(threatclaw_server, web_search_configured):
    ...
    r2 = await api_get(threatclaw_server, "/api/extensions/web-search/setup")
...
async def test_gmail_fields(threatclaw_server, gmail_installed):
    """Gmail extension has correct field values (OAuth-based auth)."""
    ext = await _get_extension(threatclaw_server, "gmail")
...
async def test_both_extensions_listed(
    threatclaw_server, web_search_configured, gmail_installed
):
...
async def test_gmail_setup_schema_auto_resolves(threatclaw_server, gmail_installed):
    """Gmail setup schema returns empty secrets (builtin creds auto-resolve)."""
    r = await api_get(threatclaw_server, "/api/extensions/gmail/setup")
    assert r.status_code == 200
    data = r.json()
...
async def test_removed_not_in_extensions(threatclaw_server, web_search_removed):
...
async def test_removed_extension_not_listed(threatclaw_server, web_search_removed):
...
async def test_removed_not_in_registry_installed(threatclaw_server, web_search_removed):
...
async def test_activate_after_remove_uses_replacement_bytes_not_cached_module(
    threatclaw_server, wasm_tools_dir, web_search_removed
):
...
async def test_reinstall_after_remove(threatclaw_server, web_search_reinstalled):
...
async def test_install_nonexistent(threatclaw_server):
...
async def test_install_empty_name(threatclaw_server):
...
async def test_remove_noninstalled(threatclaw_server):
    """Removing a non-installed extension returns an error."""
    r = await api_post(
        threatclaw_server, "/api/extensions/nonexistent-xyz/remove", timeout=10
    )
    if r.status_code == 200:
        assert r.json().get("success") is False
    else:
...
async def test_activate_noninstalled(threatclaw_server):
    """Activating a non-installed extension returns an error."""
    r = await api_post(
        threatclaw_server, "/api/extensions/nonexistent-xyz/activate", timeout=10
    )
    if r.status_code == 200:
        assert r.json().get("success") is False
    else:
...
async def test_setup_noninstalled(threatclaw_server):
    """Setup for non-installed extension returns an error."""
    r = await api_get(threatclaw_server, "/api/extensions/nonexistent-xyz/setup")
    # May return 500 or a JSON error
    assert r.status_code >= 400 or r.json().get("success") is False


async def test_configure_noninstalled(threatclaw_server):
