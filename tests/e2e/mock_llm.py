...
def _extract_tool_name(msg: dict) -> str:
    ...
    if name:
        return name
    # threatclaw wraps tool output as <tool_output name="...">
    content = msg.get("content", "")
...
async def oauth_exchange(request: web.Request) -> web.Response:
    ...
    a fake token response. Called by threatclaw's exchange_via_proxy() when
    THREATCLAW_OAUTH_EXCHANGE_URL is set.
    """
...
async def mcp_oauth_register(request: web.Request) -> web.Response:
    ...
    return web.json_response({
        ...
        "client_name": body.get("client_name", "ThreatClaw"),
