...
use threatclaw::agent::routine_engine::RoutineEngine;
use threatclaw::agent::{Agent, AgentDeps, SessionManager as AgentSessionManager};
use threatclaw::app::{AppBuilder, AppBuilderFlags};
use threatclaw::channels::IncomingMessage;
use threatclaw::channels::web::log_layer::LogBroadcaster;
use threatclaw::channels::web::server::{GatewayState, RateLimiter, start_server};
use threatclaw::channels::web::sse::SseManager;
use threatclaw::channels::web::ws::WsConnectionTracker;
use threatclaw::config::{Config, RegistryProviderConfig, RoutineConfig};
use threatclaw::db::Database;
use threatclaw::db::libsql::LibSqlBackend;
use threatclaw::llm::registry::ProviderProtocol;
use threatclaw::llm::{
    SessionConfig as LlmSessionConfig, SessionManager as LlmSessionManager, create_llm_provider,
};
use threatclaw::secrets::SecretsStore;
...
impl Tool for MockGithubWebhookTool {
    ...
    async fn execute(
        ...
        _ctx: &threatclaw::context::JobContext,
    ...
    fn webhook_capability(&self) -> Option<threatclaw::tools::wasm::WebhookCapability> {
        Some(threatclaw::tools::wasm::WebhookCapability {
...
impl GatewayWorkflowHarness {
    pub async fn start_openai_compatible(base_url: &str, model: &str) -> Self {
        ...
        let channel_manager = threatclaw::channels::ChannelManager::new();
        ...
        *scheduler_slot.write().await = Some(agent.scheduler());
        ...
        let webhook_secrets = Arc::new(threatclaw::secrets::InMemorySecretsStore::new(Arc::new(
            threatclaw::secrets::SecretsCrypto::new(SecretString::from(
        ...
        webhook_secrets
            .create(
                &user_id,
                threatclaw::secrets::CreateSecretParams::new(
        ...
        let webhook_state = threatclaw::webhooks::ToolWebhookState {
            tools: Arc::clone(gateway_state.tool_registry.as_ref().expect("tool registry")),
            routine_engine: Arc::clone(&routine_slot),
            user_id: user_id.clone(),
            secrets_store: Some(
                webhook_secrets as Arc<dyn threatclaw::secrets::SecretsStore + Send + Sync>,
            ),
        };
        let webhook_app = threatclaw::webhooks::routes(webhook_state);
