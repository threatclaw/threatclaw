...
use threatclaw::agent::{Agent, AgentDeps};
use threatclaw::app::{AppBuilder, AppBuilderFlags};
use threatclaw::channels::web::log_layer::LogBroadcaster;
use threatclaw::channels::{OutgoingResponse, StatusUpdate};
use threatclaw::config::Config;
use threatclaw::db::Database;
use threatclaw::llm::{LlmProvider, SessionConfig, SessionManager};
use threatclaw::tools::Tool;
...
use threatclaw::llm::recording::{HttpExchange, ReplayingHttpInterceptor};
...
pub struct TestRig {
    ...
    #[cfg(feature = "libsql")]
    workspace: Option<Arc<threatclaw::workspace::Workspace>>,
    ...
    #[cfg(feature = "libsql")]
    extension_manager: Option<Arc<threatclaw::extensions::ExtensionManager>>,
...
impl TestRig {
    ...
    pub async fn send_incoming(&self, msg: threatclaw::channels::IncomingMessage) {
    ...
    pub async fn run_and_verify_trace(
        ...
        if !trace.expects.is_empty() {
            ...
            for status in self.channel.captured_status_events() {
                if let threatclaw::channels::StatusUpdate::ToolCompleted {
                    name,
                    success: false,
                    error,
                    parameters,
...
impl TestRigBuilder {
    ...
    pub async fn build(self) -> TestRig {
        use threatclaw::channels::ChannelManager;
        ...
        let db: Arc<dyn threatclaw::db::Database> = Arc::new(backend);
        ...
        // 6. Register job tools, routine tools, and extra tools.
        {
            ...
            if let (Some(db_arc), Some(ws)) = (&components.db, &components.workspace) {
                ...
                use threatclaw::config::RoutineConfig;
            ...
            if enable_skills {
                let registry = Arc::new(std::sync::RwLock::new(
                    threatclaw::skills::SkillRegistry::new(temp_dir.path().join("skills"))
                        .with_installed_dir(temp_dir.path().join("installed_skills")),
                ));
                let catalog = threatclaw::skills::catalog::shared_catalog();
                components
                    .tools
            ...
            http_interceptor: {
                ...
                if exchanges.is_empty() {
                    ...
                    Some(Arc::new(ReplayingHttpInterceptor::new(exchanges))
                        as Arc<dyn threatclaw::llm::recording::HttpInterceptor>)
                }
            },
            transcription: None,
        ...
        let routine_config = if enable_routines {
            Some(threatclaw::config::RoutineConfig {
                enabled: true,
...
impl TestRig {
    ...
    #[cfg(feature = "libsql")]
    pub fn workspace(&self) -> Option<&Arc<threatclaw::workspace::Workspace>> {
