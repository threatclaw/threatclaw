...
mod advanced {
    ...
    use threatclaw::agent::routine::Trigger;
    ...
    async fn wait_for_routine_run(
        ...
        timeout: Duration,
    ) -> Vec<threatclaw::agent::routine::RoutineRun> {
    ...
    async fn user_steering() {
        let _cleanup = CleanupGuard::new().file("/tmp/threatclaw_steer_test.txt");
        let _ = std::fs::remove_file("/tmp/threatclaw_steer_test.txt");
        ...
        let content = std::fs::read_to_string("/tmp/threatclaw_steer_test.txt")
    ...
    async fn tool_error_recovery() {
        let _cleanup = CleanupGuard::new().file("/tmp/threatclaw_recovery_test.txt");
        let _ = std::fs::remove_file("/tmp/threatclaw_recovery_test.txt");
        ...
        let content = std::fs::read_to_string("/tmp/threatclaw_recovery_test.txt")
    ...
    async fn long_tool_chain() {
        let test_dir = "/tmp/threatclaw_chain_test";
        ...
        rig.send_message(
            "Create a daily log at /tmp/threatclaw_chain_test/log.md, \
    ...
    async fn routine_news_digest() {
        use threatclaw::llm::recording::{HttpExchange, HttpExchangeRequest, HttpExchangeResponse};
    ...
    async fn mcp_extension_lifecycle() {
        ...
        use threatclaw::extensions::{AuthHint, ExtensionKind, ExtensionSource, RegistryEntry};
        ...
        ext_mgr
            ...
            .create(
                "default",
                threatclaw::secrets::CreateSecretParams::new(secret_name, "mock-access-token")
