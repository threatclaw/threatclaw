...
import {
  ...
  Clock, Filter, Search, X, Eye, FileText,
} from "lucide-react";
                  ...
                  <div style={{ marginTop: "14px", borderTop: "1px solid var(--tc-border-light)", paddingTop: "14px" }}>
                    ...
                    {f.metadata && (
                      <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", marginBottom: "10px" }}>
                        {f.metadata.agent_ip && (
                          <span style={{ fontSize: "10px", padding: "3px 8px", borderRadius: "var(--tc-radius-sm)", background: "rgba(48,128,208,0.08)", color: "var(--tc-blue)", border: "1px solid rgba(48,128,208,0.15)", fontFamily: "monospace" }}>
                            {String(f.metadata.agent_ip)}
                          </span>
                        )}
                        {f.metadata.src_ip && (
                          <span style={{ fontSize: "10px", padding: "3px 8px", borderRadius: "var(--tc-radius-sm)", background: "rgba(232,64,64,0.08)", color: "var(--tc-red)", border: "1px solid rgba(232,64,64,0.15)", fontFamily: "monospace" }}>
                            src: {String(f.metadata.src_ip)}
                          </span>
                        )}
                        {f.metadata.scan_target && (
                        ...
                        {f.metadata.mitre && Array.isArray(f.metadata.mitre) && f.metadata.mitre.length > 0 && (
                          <span style={{ fontSize: "10px", padding: "3px 8px", borderRadius: "var(--tc-radius-sm)", background: "rgba(128,64,208,0.08)", color: "#8040d0", border: "1px solid rgba(128,64,208,0.15)", fontFamily: "monospace" }}>
                            MITRE: {(f.metadata.mitre as string[]).join(", ")}
                          </span>
                        )}
                    ...
                    <div style={{ display: "flex", gap: "6px" }}>
                      ...
                      <ChromeButton onClick={async () => {
                        try {
                          const res = await fetch("/api/tc/connectors/glpi/ticket", {
