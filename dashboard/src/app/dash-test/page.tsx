...
export default function DashTest() {
  ...
  return (
    ...
    >
      {/* ═══════ TOP BAR ═══════ */}
      <TopBar now={now} criticalCount={confirmedHigh.length + pending.filter((i) => isSevere(i.severity)).length} />
...
// TOP BAR
// ═══════════════════════════════════════════════════════════════
function TopBar({ now, criticalCount }: { now: Date; criticalCount: number }) {
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
        <img src="/logo.png" alt="" width={20} height={20} style={{ borderRadius: "4px" }} />
        <div>
          <div style={{ fontSize: "11px", letterSpacing: "0.18em", fontWeight: 600, textTransform: "uppercase" }}>
            threat<span style={{ color: "var(--tc-red)", margin: "0 4px" }}>/</span>claw
          </div>
          <div style={{ fontSize: "9px", color: "var(--tc-text-muted)", letterSpacing: "0.14em" }}>
            staging · 10.10.10.2
          </div>
        </div>
      </div>
      ...
      <nav style={{ display: "flex", gap: 0, alignSelf: "stretch" }}>
        <NavTab href="/dash-test" label="Console" active />
        <NavTab href="/incidents" label="Incidents" />
        <NavTab href="/chat" label="Chat" />
        <NavTab href="/sources" label="Sources" />
        <NavTab href="/intelligence" label="Intelligence" />
        <NavTab href="/governance" label="Governance" />
        <NavTab href="/exports" label="Reports" />
      </nav>

      <div style={{ display: "flex", alignItems: "center", gap: "14px", fontSize: "10px" }}>
        <span style={{ display: "inline-flex", alignItems: "center", gap: "6px", border: "1px solid var(--tc-border)", padding: "3px 8px" }}>
          <Pulse color="#30a050" />
          engine · live
        </span>
        ...
        {criticalCount > 0 && (
          ...
          >
            ...
            {criticalCount} incident{criticalCount > 1 ? "s" : ""} HIGH+
          </span>
        )}
      </div>
    </header>
  ...
  return (
    <Link
      ...
      style={{
        ...
        borderLeft: active ? "none" : undefined,
