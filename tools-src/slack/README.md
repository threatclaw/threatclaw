...
A standalone WASM component that provides Slack integration for ThreatClaw. This serves as both a functional tool and a template for building custom WASM tools.
...
```bash
mkdir -p ~/.threatclaw/tools
cp target/wasm32-wasip2/release/slack_tool.wasm ~/.threatclaw/tools/slack.wasm
cp slack.capabilities.json ~/.threatclaw/tools/
```
...
```bash
threatclaw secret set slack_bot_token "xoxb-your-token-here"
```
